...
(what the document pipeline produces) reports that it does not read as text and
offers the whole file as a download. The download happens in the page: the
bytes arrive base64 in the read answer and become an object URL, so no
unfenced byte-serving route is exposed to the audience.
