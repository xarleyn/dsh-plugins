...
- Логирование — `createLogger` из `plugin-kit` (префикс `[dsh:<name>]`) или
  Diagnostics-паттерн с кодами (§3.6). В логах нет секретов, токенов и
  содержимого пользовательских промптов без явной необходимости.
