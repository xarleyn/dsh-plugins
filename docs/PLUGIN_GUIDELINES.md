...
- Логирование — обёртка над `pino` из `src/logging/` плагина (механизм,
  формат и рецепт подключения — [PLUGIN_LOGGING.md](./PLUGIN_LOGGING.md)):
  NDJSON-файлы в `<$DSH_HOME>/logs/<id>/`, уровни, ежедневная ротация,
  консольное зеркало `warn+`. В логах нет секретов, токенов и содержимого
  пользовательских промптов без явной необходимости.
