...
describe("validateConfig", () => {
    it("should return empty object for undefined optional fields", () => {
        const result = validateConfig({}, { name: "string", count: "number" });
        expect(result).toEqual({});
    });
    it("should validate correct types", () => {
        const result = validateConfig({ name: "test", count: 42, enabled: true, meta: {} }, {
            name: "string",
            count: "number",
            enabled: "boolean",
            meta: "object",
    ...
    it("should throw on wrong type", () => {
        expect(() => validateConfig({ name: 123 }, { name: "string" })).toThrow('Invalid type for "name": expected string, got number');
    });
    it("should reject null for object fields", () => {
        expect(() => validateConfig({ meta: null }, { meta: "object" })).toThrow('Invalid type for "meta": expected object, got null');
    });
    it("should throw when config is not an object", () => {
        ...
        expect(() => validateConfig(42, {})).toThrow("Configuration must be an object");
    });
});
