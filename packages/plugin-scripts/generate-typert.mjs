...
export async function generateTypert(options) {
  ...
  const compilerOptions = {
    ...
    paths: {
      "@deepseek-ai/dsh-typert-protocol": [
        "packages/dsh-typert-protocol/src/index.d.ts",
      ],
    },
    types: ["node"],
  };
  writeFileSync(
    join(stagedRoot, "tsconfig.json"),
    `${JSON.stringify(
      {
        compilerOptions,
        include: ["src/**/*.ts"],
  ...
  if (artifact === undefined || artifact.remote === undefined) {
    throw new Error(
      `Typert did not discover the ${options.serviceName} Remote service: ${JSON.stringify(
        face?.packages.map(({ name, exports, services, invocations }) => ({
          ...
          invocations: invocations.map(
            (item) => `${item.namespace}/${item.method}`,
          ),
        })),
      )}`,
    );
