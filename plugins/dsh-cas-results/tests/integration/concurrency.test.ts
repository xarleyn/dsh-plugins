...
describe("concurrent duplicate writes (SPEC AC10)", () => {
  it("stores exactly one valid blob for 100 concurrent identical puts", async () => {
    ...
    expect(reused + outcomes.filter((object) => !object.reused).length).toBe(
      100,
    );
    // No staging residue.
  ...
  it("keeps distinct payloads intact under parallel writes", async () => {
    ...
    const payloads = Array.from({ length: 24 }, (_, index) =>
      encoder.encode(`unique payload ${index}: ${makeLog(1_000)}`),
    );
    const outcomes = await Promise.all(
    ...
    for (let index = 0; index < payloads.length; index += 1) {
      ...
      expect(
        Buffer.from(read.bytes).equals(
          Buffer.from(payloads[index] as Uint8Array),
        ),
      ).toBe(true);
    }
  });
