...
async function deps(): Promise<Deps> {
  ...
  const config = resolveCasResultsConfig({
    retrieval: { defaultBytes: 32_768, maxBytes: 65_536 },
  });
...
describe("dsh_cas_retrieve", () => {
  it("returns byte-identical content for text payloads (SPEC AC2)", async () => {
    ...
    const object = await d.store.put({
      payload,
      kind: "text",
      mediaType: "text/plain",
      encoding: "utf8",
    });
    const outcome = (await execute(d.retrieve, { ref: object.ref })) as {
      content: string;
      totalBytes: number;
      truncated: boolean;
    };
    ...
    const rest = (await execute(d.retrieve, {
      ref: object.ref,
      offset: Buffer.byteLength(outcome.content, "utf8"),
    })) as { content: string };
  ...
  it("paginates large payloads and reports the next offset", async () => {
    ...
    const first = (await execute(d.retrieve, { ref: object.ref })) as {
      offset: number;
    ...
    const second = (await execute(d.retrieve, {
      ref: object.ref,
      offset: first.returnedBytes,
    })) as { offset: number };
  ...
  it("enforces the configured hard maximum regardless of the requested limit (SPEC AC6)", async () => {
    ...
    const outcome = (await execute(d.retrieve, {
      ref: object.ref,
      limit: 10_000_000,
    })) as { returnedBytes: number };
  ...
  it("returns canonical base64 for binary payloads", async () => {
    ...
    const auto = (await execute(d.retrieve, { ref: object.ref })) as {
      encoding: string;
      content: string;
    };
    expect(auto.encoding).toBe("base64");
    expect(
      Buffer.from(auto.content, "base64").equals(Buffer.from(TINY_PNG_BYTES)),
    ).toBe(true);
    const hex = (await execute(d.retrieve, {
      ref: object.ref,
      encoding: "hex",
    })) as { content: string };
    expect(hex.content.slice(0, 8)).toBe(
      Buffer.from(TINY_PNG_BYTES).toString("hex").slice(0, 8),
    );
  });
  ...
  it("rejects malformed refs and missing objects with actionable messages (SPEC §24)", async () => {
    const d = await deps();
    await expect(
      execute(d.retrieve, { ref: "not-a-ref" }),
    ).rejects.toThrowError(/invalid CAS reference/);
    const missing = `sha256:${sha256Hex(encoder.encode("never stored"))}`;
    await expect(execute(d.retrieve, { ref: missing })).rejects.toThrowError(
      /no longer available/,
    );
  });
});
...
describe("dsh_cas_search", () => {
    ...
    for (let index = 0; index < 20_000; index += 1) {
      lines.push(
        index === 10_000
          ...
          : `2026-08-30T18:00:00.000Z INFO line ${index}`,
      );
    }
    const object = await d.store.put({
      payload: encoder.encode(lines.join("\n")),
      kind: "log",
      mediaType: "text/log",
      encoding: "utf8",
    });
    const outcome = (await execute(d.search, {
      ref: object.ref,
  ...
  it("requires a non-empty query", async () => {
    const d = await deps();
    await expect(
      execute(d.search, { ref: `sha256:${"a".repeat(64)}`, query: "" }),
    ).rejects.toThrowError(/query/);
  });
});
...
describe("dsh_cas_info and dsh_cas_stats", () => {
  it("reports metadata and aggregates", async () => {
    const d = await deps();
    const object = await d.store.put({
      payload: encoder.encode(makeText(50_000)),
      kind: "text",
      mediaType: "text/plain",
      encoding: "utf8",
      firstTool: "bash",
    ...
    const info = (await execute(d.info, { ref: object.ref })) as {
      exists: boolean;
      size: number;
      firstTool: string;
      kind: string;
    ...
    const missing = (await execute(d.info, {
      ref: `sha256:${"b".repeat(64)}`,
    })) as { exists: boolean; message: string };
    expect(missing.exists).toBe(false);
    expect(missing.message).toContain("no longer available");

    const stats = (await execute(d.stats, {})) as {
      objects: number;
      logicalBytes: number;
    };
...
describe("dsh_cas_gc", () => {
  it("runs a collection pass and reports the outcome", async () => {
    const d = await deps();
    const object = await d.store.put({
      payload: encoder.encode("to be collected"),
      kind: "text",
      mediaType: "text/plain",
      encoding: "utf8",
    });
    await d.store.touch(
      object.ref.replace("sha256:", ""),
      new Date(Date.parse("2020-01-01T00:00:00.000Z")),
    );
