...
import {
  classifyText,
  sniffBinaryMediaType,
} from "../../src/transform/classify.js";
import { htmlToText } from "../../src/preview/html.js";

describe("classifyText", () => {
  it("recognizes HTML documents", () => {
    expect(classifyText("<!DOCTYPE html><html><body>x</body></html>")).toEqual({
      kind: "html",
      mediaType: "text/html",
    });
  ...
  it("keeps ordinary text generic", () => {
    expect(classifyText("just a plain sentence")).toEqual({
      kind: "text",
...
describe("sniffBinaryMediaType", () => {
  it("detects common magic headers", () => {
    ...
    expect(sniffBinaryMediaType(new TextEncoder().encode("plain"))).toBe(
