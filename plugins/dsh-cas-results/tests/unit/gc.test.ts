...
describe("garbage collection", () => {
  it("deletes expired objects but protects fresh ones (SPEC §24)", async () => {
    ...
    const expired = await storeObject(
      store,
      "expired payload",
      new Date(now - 40 * 24 * 3_600_000),
    );
    const fresh = await storeObject(
      store,
      "fresh payload",
  ...
  it("evicts oldest-accessed objects when the store exceeds its quota (SPEC AC11)", async () => {
    const { store } = await world();
    const now = Date.parse("2026-09-01T00:00:00.000Z");
    const old = await storeObject(
      store,
      "x".repeat(800),
      new Date(now - 72 * 3_600_000),
    );
    const young = await storeObject(
      store,
      "y".repeat(400),
      new Date(now - 1_000),
    );
    ...
    expect(
      await store.stats().then((stats) => stats.logicalBytes),
  ...
  it("cleans orphan staging files older than an hour", async () => {
    ...
    await store.put({
      payload: encoder.encode("warm the tmp dir"),
      kind: "text",
      mediaType: "text/plain",
      encoding: "utf8",
    });
