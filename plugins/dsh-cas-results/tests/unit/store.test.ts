...
describe("FilesystemCasStore.read", () => {
  ...
  it("detects tampered payloads via integrity verification (SPEC §26)", async () => {
    ...
    const store = buildStore(root);
    const object = await store.put({
      payload: encoder.encode("integrity matters"),
      kind: "text",
      mediaType: "text/plain",
      encoding: "utf8",
    });
  ...
  it("throws CAS_OBJECT_MISSING for unknown or GC-cleaned hashes", async () => {
    ...
    const error = (await store
      .read(missing)
      .catch((caught: unknown) => caught)) as CasError;
  ...
  it("refuses invalid hashes before touching the filesystem", async () => {
    const root = await tempRoot();
    const store = buildStore(root);
    await expect(store.has("nope")).rejects.toThrowError(
...
describe("FilesystemCasStore.stat/touch", () => {
  ...
  it("touch moves lastAccessedAt forward", async () => {
    ...
    expect(meta?.lastAccessedAt).toBe(
...
describe("FilesystemCasStore.stats", () => {
  it("aggregates object counts and byte totals", async () => {
    ...
    const store = buildStore(root);
    await store.put({
      payload: encoder.encode("a".repeat(100)),
      kind: "text",
      mediaType: "text/plain",
      encoding: "utf8",
    });
    await store.put({
      payload: encoder.encode("b".repeat(300)),
      kind: "log",
      mediaType: "text/log",
      encoding: "utf8",
    });
...
describe("FilesystemCasStore.search", () => {
  it("finds matches with context and honors caps", async () => {
    ...
    const caseSensitive = await store.search(hash, {
      query: "AssertionError",
      caseSensitive: true,
    });
  ...
  it("rejects empty queries", async () => {
    ...
    await expect(
      store.search(parseCasRef(object.ref), { query: "" }),
