...
export function normalizeConfig(
  ...
  const defaults =
    raw.defaults === undefined ? {} : expectRecord(raw.defaults, "defaults");
  const mode = enumValue(
    defaults.mode,
    RESOLUTION_MODES,
    "defaults.mode",
    fallbacks.mode ?? DEFAULT_MODE,
  );
  const scope = enumValue(
    defaults.scope,
    SCOPES,
    "defaults.scope",
    fallbacks.scope ?? DEFAULT_SCOPE,
  );
