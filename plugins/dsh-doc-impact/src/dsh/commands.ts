 ...
 */
export function createDocImpactCommand(
  engine: EngineFacade,
  ...
  return {
    name: "doc-impact",
    description:
      "Documentation impact status and debugging (dsh-doc-impact plugin).",
    ...
    async handler(
      ...
      if (typeof cwd !== "string" || cwd === "") {
        return {
          kind: "error",
          text: "This session has no working directory; dsh-doc-impact is not active.",
        };
      }
      ...
      if (raw === "") {
        ...
        return {
          kind: "success",
          text: formatStatus(status.pending, status.resolved),
        };
      }
      ...
      if (subcommand === "check") {
        ...
        if (decision.pending.length === 0) {
          return {
            kind: "success",
            text: "No documentation impacts for the current changes.",
          };
        }
        return {
          kind: "success",
      ...
      if (subcommand === "explain") {
        ...
        if (rules.length === 0) {
          return {
            kind: "error",
            text: `No dsh-doc-impact workspace config found under ${cwd}.`,
          };
        }
        ...
        return {
          ...
          text: formatExplain(
            rule,
            decision.pending.some((impact) => impact.ruleId === rule.id),
          ),
        };
      }
