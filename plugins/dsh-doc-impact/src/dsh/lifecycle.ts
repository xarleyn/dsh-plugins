 ...
 */
export function currentTurnNumber(events: readonly SessionEventLike[]): {
  turn: number;
  open: boolean;
} {
  let turn = 0;
...
export function registerLifecycle(
  ...
  ctx.on("session/event", ((
    ...
    if (event.type === "turn/start" && typeof data.turn === "number") {
      void engine
        .ensureBaseline(sessionId, cwd, data.turn)
        .catch(() => undefined);
  ...
  ctx.on("agent/turn-stopping", (async (payload: {
    ...
    try {
      ...
      ctx.logger.warn(
        "dsh-doc-impact: turn-stopping check failed; allowing stop (%s)",
        error,
      );
    }
  }) as never);
