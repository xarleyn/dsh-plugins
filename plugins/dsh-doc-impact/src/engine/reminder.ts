...
function relationLabel(impact: Impact): string {
  const source =
    impact.triggerSide === "code" ? "implementation" : "documentation";
  const target =
    impact.targetSide === "docs" ? "documentation" : "implementation";
...
export function buildReminderMessage(
  ...
  const blocks = impacts.map((impact, index) =>
    impactBlock(impact, index + 1, knownFiles),
  );
  const count = `${impacts.length} rule${impacts.length === 1 ? " was" : "s were"} triggered.`;
  const body = blocks.join("\n\n");
  const tail =
    impacts.length === 1 && impacts[0]?.mode === "remind"
      ? REMIND_TAIL
      : STRICT_TAIL;
...
export function formatStatus(
  pending: readonly Impact[],
  resolved: readonly Impact[],
): string {
  if (pending.length === 0 && resolved.length === 0) {
    return "Documentation impacts: none.";
  }
  const lines = [
    `Documentation impacts — pending: ${pending.length}, resolved: ${resolved.length}`,
  ];
  for (const impact of pending) {
    lines.push(
      `  pending  ${impact.ruleId} → ${impact.targetFiles.join(", ")}`,
    );
  }
  for (const impact of resolved) {
    lines.push(
      `  ${impact.status.padEnd(16)} ${impact.ruleId} → ${impact.targetFiles.join(", ")}`,
    );
  }
  return lines.join("\n");
}
...
export function formatExplain(rule: ImpactRule, impacted: boolean): string {
  const lines = [
    ...
    impacted
      ? "This rule is currently triggered by this agent's changes."
      : "This rule is not currently triggered.",
  ];
