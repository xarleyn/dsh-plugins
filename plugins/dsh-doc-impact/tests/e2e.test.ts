...
async function makeRepo(): Promise<string> {
  ...
  await writeFile(
    join(cwd, "src", "auth", "session.ts"),
    "export const session = 1;\n",
  );
  await writeFile(
    join(cwd, "docs", "authentication.md"),
...
async function makeHarness(userDirtyDoc = false): Promise<Harness> {
  ...
  return {
    ...
    async preStep() {
      ...
      await listener?.(
        { agent, turn: 1, step: 1, signal: undefined },
        async () => ({}),
      );
    },
    async stop() {
    ...
    async command(raw: string) {
      const command = createDocImpactCommand(engine, {
        rulesFor: async () =>
          (await source(cwd))?.config.rules ?? ([] as ImpactRule[]),
      });
