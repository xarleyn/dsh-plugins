...
function changed(
  detector: ReturnType<typeof createGitDetector>,
  cwd: string,
  baseline: Awaited<ReturnType<typeof detector.captureBaseline>>,
) {
...
afterAll(async () => {
  await Promise.all(
    repos.map((repo) => rm(repo, { recursive: true, force: true })),
  );
});
...
describe("git baseline detector", () => {
  it("detects agent modifications in a clean workspace", async () => {
    ...
    expect(
      [...paths(diff).entries()].sort(([a], [b]) => a.localeCompare(b)),
    ).toEqual([
  ...
  it("falls back to filesystem enumeration when git is absent", async () => {
    ...
    repos.push(plain);
    const { createFilesystemDetector } =
      await import("../src/changes/filesystem-detector.js");
    ...
    expect(
      await detector.listFiles(plain).then((files) => files.sort()),
