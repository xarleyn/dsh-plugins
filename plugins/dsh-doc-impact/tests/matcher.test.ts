...
describe("impact matching", () => {
  ...
  it("automatically satisfies an impact when a target changed too", () => {
    ...
    const [impact] = matchImpacts(config, [
      "src/auth/session.ts",
      "docs/auth.md",
    ]);
  ...
  it("implements docs-to-code semantics", () => {
    ...
    const [impact] = matchImpacts(config, ["docs/configuration.md"], {
  ...
  it("emits explainable impacts in both directions", () => {
    ...
    const impacts = matchImpacts(config, [
      "src/contract.ts",
      "docs/contract.md",
    ]);
    expect(
      impacts.map(({ triggerSide, targetSide, status }) => ({
        triggerSide,
        targetSide,
        status,
      })),
