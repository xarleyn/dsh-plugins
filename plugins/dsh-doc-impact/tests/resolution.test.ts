...
describe("fingerprints and resolution", () => {
  it("creates stable order-independent fingerprints", () => {
    const first = createImpactFingerprint(
      "auth",
      ["b.ts", "a.ts"],
      ["z.md", "x.md"],
    );
    const second = createImpactFingerprint(
      "auth",
      ["a.ts", "b.ts"],
      ["x.md", "z.md"],
    );
  ...
  it("automatically resolves a changed target", () => {
    expect(autoResolveImpact(pendingImpact(), ["docs\\auth.md"]).status).toBe(
      "updated",
    );
    expect(autoResolveImpact(pendingImpact(), ["src/auth.ts"]).status).toBe(
      "pending",
    );
  });
  ...
  it("validates explicit resolutions", () => {
    ...
    expect(() =>
      resolveImpact(pendingImpact(), { ruleId: "auth", status: "updated" }, []),
    ...
    expect(
      resolveImpact(
        pendingImpact(),
        {
          ...
          reason: "Purely internal refactor",
        },
        [],
      ),
    ).toMatchObject({
...
describe("reminder state", () => {
  ...
  it("supersedes an old pending fingerprint when a rule changes shape", () => {
    ...
    const nextImpact = pendingImpact({
      id: "new",
      triggerFiles: ["src/auth-v2.ts"],
    });
