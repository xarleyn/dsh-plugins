...
Status: Draft  
Target: `qa-surface`  
> Примечание: реализация этой спецификации переехала из `qa-surface` в отдельный плагин `@yadsh/dsh-documents`; ниже — исходная спецификация.  
...
```text
DSH_DOCUMENTS_ENABLED=true
DSH_DOCUMENTS_STORAGE_ROOT=...
DSH_DOCUMENTS_DOCLING_BASE_URL=http://docling:5001
DSH_DOCUMENTS_MAX_INPUT_BYTES=52428800
```
