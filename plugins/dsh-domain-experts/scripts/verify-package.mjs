...
assert.ok(
  compatibility.deepseekHarness?.requiredClientFeatures?.includes(
...
const remote = await readFile(
  new URL("../lib/typert.remote-client.js", import.meta.url),
  "utf8",
);
for (const method of [
...
const hostRemote = await readFile(
  new URL("../lib/typert.host.js", import.meta.url),
  "utf8",
);
...
assert.match(
  client,
  new RegExp(
    `__ModuleLoader__\\.load\\(\\{\\s*id:\\s*${JSON.stringify(PACKAGE_NAME)}`,
    "u",
  ),
...
assert.match(
  ...
  "client bundle must register a settings tab",
);
assert.match(client, /domain-experts/u, "client bundle must carry the tab id");
assert.match(
  client,
  /Domain Experts/u,
  "client bundle must carry the tab label",
);
assert.doesNotMatch(
  client,
  /dsh-plugin-card/u,
