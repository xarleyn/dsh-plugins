...
describe("delegation policy", () => {
  it("allows expert-only access to another enabled domain", () => {
    ...
    expect(verdict).toMatchObject({ allowed: true, mode: "expert-only", message: "" });
  });

  it("refuses a caller that no longer exists", () => {
    const verdict = delegationVerdictOf(undefined, "ghost", domainOf("inventory"), "inventory");
  ...
  it("refuses every target when cross-domain access is off", () => {
    const caller = domainOf("payments", {
      delegation: { ...domainOf("payments").delegation, allowCrossDomain: false },
    });
    const verdict = delegationVerdictOf(caller, "payments", domainOf("inventory"), "inventory");
  ...
  it("refuses every target in disabled mode", () => {
    const caller = domainOf("payments", {
      delegation: { ...domainOf("payments").delegation, crossDomainMode: "disabled" },
    });
    expect(
      delegationVerdictOf(caller, "payments", domainOf("inventory"), "inventory").allowed,
    ).toBe(false);
  });
  ...
  it("enforces an explicit target list", () => {
    const caller = domainOf("payments", {
      delegation: { ...domainOf("payments").delegation, targets: ["inventory"] },
    });
    expect(
      delegationVerdictOf(caller, "payments", domainOf("inventory"), "inventory").allowed,
    ).toBe(true);
    const refused = delegationVerdictOf(caller, "payments", domainOf("platform"), "platform");
  ...
  it("refuses a missing or disabled target", () => {
    const caller = domainOf("payments");
    expect(delegationVerdictOf(caller, "payments", undefined, "ghost").allowed).toBe(false);
    expect(
      delegationVerdictOf(caller, "payments", domainOf("platform", { enabled: false }), "platform")
        .allowed,
    ).toBe(false);
  });
  ...
  it("allows direct-read mode as well", () => {
    const caller = domainOf("payments", {
      delegation: { ...domainOf("payments").delegation, crossDomainMode: "direct-read" },
    });
    expect(
      delegationVerdictOf(caller, "payments", domainOf("inventory"), "inventory"),
...
describe("parallel budget", () => {
  it("admits runs below the limit", () => {
    expect(parallelBudgetOf(0, 3)).toEqual({ exceeded: false, limit: 3, active: 0 });
