...
export function validateRepoRelativePath(
  ...
  if (input.length > MAX_PATH_LENGTH) {
    throw new GitToolError(
      "invalid-path",
      `${field} is longer than ${MAX_PATH_LENGTH} characters`,
    );
  }
  ...
  if (normalized.startsWith("/")) {
    throw new GitToolError(
      "invalid-path",
      `${field} must be repository-relative, not absolute`,
    );
  }
  if (/^[a-zA-Z]:/.test(normalized)) {
    throw new GitToolError(
      "invalid-path",
      `${field} must be repository-relative, not a drive path`,
    );
  }
  ...
  for (const segment of segments) {
    if (segment === "..") {
      throw new GitToolError(
        "invalid-path",
        `${field} must not traverse outside the repository`,
      );
    }
    if (segment.startsWith("-")) {
      throw new GitToolError(
        "invalid-path",
        `${field} segments must not start with "-"`,
      );
    }
  }
...
export function validateSearchLiteral(input: unknown, field: string): string {
  ...
  if (value.length > MAX_LITERAL_LENGTH) {
    throw new GitToolError(
      "invalid-argument",
      `${field} is longer than ${MAX_LITERAL_LENGTH} characters`,
    );
  }
  return value;
}
...
export function clampLineRange(
  ...
  if (!Number.isFinite(from) || from < 1) {
    throw new GitToolError(
      "invalid-argument",
      "fromLine must be a positive integer",
    );
  }
  ...
  if (!Number.isFinite(to) || to < from) {
    throw new GitToolError(
      "invalid-argument",
      "toLine must be an integer greater than or equal to fromLine",
    );
  }
  let truncated = false;
