  ...
  return defineTool({
    ...
    parameters: {
      ...
      toLine: {
        type: "integer",
        description:
          "Last line to attribute (inclusive). Default: configured per-call cap from fromLine.",
      },
      oid: {
        type: "string",
        description:
          "Hexadecimal commit id to blame an older revision of the file.",
      },
    },
    output: {
      schema: {
      ...
      render: (_args, value: GitBlameResult) => [
        {
          ...
          text: [
            ...
            ...value.lines.map(
              (blame) =>
                `${blame.line} ${blame.oid.slice(0, 8)} ${blame.author}: ${blame.content}`,
            ),
          ].join("\n"),
        },
      ],
    ...
    async execute(args: Record<string, unknown>, exec: ToolExec) {
      ...
      const repoRoot = await resolveToolRepository(
        run,
        exec,
        args["repository"],
        {
      ...
      const file = validateRepoRelativePath(args["file"], "file", {
        required: true,
      });
      if (file === undefined) {
      ...
      const oid =
        args["oid"] === undefined || args["oid"] === null
          ? undefined
          : validateCommitOid(args["oid"], "oid");
      const commit =
        oid === undefined
          ? undefined
          : await canonicalizeCommit(run, repoRoot, oid, { timeoutMs });

      const defaultedTo =
        args["toLine"] === undefined || args["toLine"] === null;
      ...
      if (
        !outputTruncated &&
        defaultedTo &&
        lines.length === range.to - range.from + 1
      ) {
        outputTruncated = true;
      }
