  ...
  return defineTool({
    ...
    parameters: {
      ...
      path: {
        type: "string",
        description:
          "Repository-relative file or directory to restrict history to.",
      },
      query: {
        type: "string",
      ...
      author: {
        type: "string",
        description:
          "Match commits authored by this name or email (literal, case-insensitive).",
      },
      limit: {
        type: "integer",
      ...
      offset: {
        type: "integer",
        description:
          "Skip this many matching commits before the page. Default 0.",
      },
      all: {
        type: "boolean",
        description:
          "Search all refs (including other branches) instead of HEAD only. Default false.",
      },
    },
    output: {
      schema: {
    ...
    async execute(args: Record<string, unknown>, exec: ToolExec) {
      ...
      if (args["author"] !== undefined && args["author"] !== null) {
        ...
        authorFilters = [
          "--regexp-ignore-case",
          `--author=${escapeRegExpLiteral(author)}`,
        ];
      }
      ...
      const page: GitHistoryResult = {
        ...
        truncated:
          result.truncated ||
          records.filter((line) => line !== "").length > limit,
      };
      return page;
    },
  });
