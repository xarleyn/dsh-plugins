  ...
  return defineTool({
    ...
    parameters: {
      ...
      oid: {
        ...
        required: true,
        description:
          "Hexadecimal commit id (7+ chars), e.g. from dsh_git_history.",
      },
      path: {
        type: "string",
    ...
    async execute(args: Record<string, unknown>, exec: ToolExec) {
      ...
      const commit = await canonicalizeCommit(run, repoRoot, oid, {
        timeoutMs,
      });
      ...
      const record = parseFormatRecord(
        metaResult.stdout.replace(/\r$/, "").replace(/\n$/, ""),
        6,
      );
