...
const runAllTools = async () => {
  ...
  const steps: [string, () => Promise<unknown>][] = [
    ...
    [
      "history",
      () =>
        tools.history.execute(
          { query: "hello", search: "content" },
          makeExec(repo.dir),
        ),
    ],
    ...
    [
      "blame",
      () => tools.blame.execute({ file: "hello.txt" }, makeExec(repo.dir)),
    ],
  ];
...
describe("read-only guarantee", () => {
  ...
  it("snapshots a repository that schedules no background work", async () => {
    ...
    // wrote.
    expect(
      (await repo.run(["config", "--get", "maintenance.auto"])).trim(),
    ).toBe("false");
  ...
  it("still produces correct output in the hostile repository", async () => {
    const tools = makeTools();
    const context = (await tools.context.execute(
      {},
      makeExec(repo.dir),
    )) as GitContextResult;
    expect(normalizePath(context.root)).toBe(normalizePath(repo.dir));

    const shown = (await tools.show.execute(
      { oid: helloOid },
      makeExec(repo.dir),
    )) as GitShowResult;
    ...
    expect(shown.files).toEqual([
      { path: "hello.txt", additions: 2, deletions: 0 },
    ]);
  });
});
...
describe("fail-closed boundaries", () => {
  ...
  it("fails closed without a session cwd", async () => {
    const tools = makeTools();
    await expect(
      tools.context.execute({}, makeExec(undefined)),
