...
beforeAll(async () => {
  ...
  oids.push(
    await repo.commit("lines.txt", "line one\nline two\n", "c2: line two"),
  );
  oids.push(
    await repo.commit(
      "lines.txt",
...
describe("dsh_git_blame", () => {
  it("attributes each line to the commit that introduced it", async () => {
    ...
    expect(blame.lines[2]).toMatchObject({
      line: 3,
      oid: oids[2],
      summary: "c3: line three",
    });
  ...
  it("rejects traversal, option-like files and bad ids before spawning", async () => {
    const tool = makeTool();
    await expect(
      tool.execute({ file: "../secret.txt" }, makeExec(repo.dir)),
    ).rejects.toThrowError(GitToolError);
    await expect(
      tool.execute({ file: "-flag" }, makeExec(repo.dir)),
    ).rejects.toThrowError(GitToolError);
    await expect(
