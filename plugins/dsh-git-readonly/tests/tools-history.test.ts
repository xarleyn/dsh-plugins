...
describe("dsh_git_history", () => {
  ...
  it("pages with limit/offset and reports truncation", async () => {
    ...
    const offsetPage = (await tool.execute(
      { limit: 2, offset: 2 },
      makeExec(repo.dir),
    )) as GitHistoryResult;
  ...
  it("filters by path, author and all refs", async () => {
    ...
    const byAuthor = (await tool.execute(
      { author: "Other Dev" },
      makeExec(repo.dir),
    )) as GitHistoryResult;
    expect(byAuthor.count).toBe(0);
    const headOnly = (await tool.execute(
      { all: false },
      makeExec(repo.dir),
    )) as GitHistoryResult;
    const allRefs = (await tool.execute(
      { all: true },
      makeExec(repo.dir),
    )) as GitHistoryResult;
    expect(allRefs.count).toBeGreaterThan(headOnly.count);
    expect(
      allRefs.commits.some((commit) => commit.subject === "c4: feature work"),
    ).toBe(true);
  });
  ...
  it("rejects invalid inputs before spawning git", async () => {
    ...
    await expect(
      tool.execute({ limit: 0 }, makeExec(repo.dir)),
    ).rejects.toThrowError(GitToolError);
    await expect(
      tool.execute({ path: "../outside" }, makeExec(repo.dir)),
    ).rejects.toThrowError(GitToolError);
  });
