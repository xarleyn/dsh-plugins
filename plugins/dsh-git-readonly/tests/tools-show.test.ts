...
describe("dsh_git_show", () => {
  it("returns structured metadata, files and patch for one commit", async () => {
    ...
    expect(shown.files).toEqual([
      { path: "src/b.txt", additions: 1, deletions: 0 },
    ]);
  ...
  it("handles the root commit", async () => {
    ...
    expect(shown.files).toEqual([
      { path: "src/a.txt", additions: 1, deletions: 0 },
    ]);
    expect(shown.patch).toContain("diff --git");
  });
  ...
  it("rejects option strings, non-hex refs and unknown ids", async () => {
    ...
    for (const hostile of [
      ...
      await expect(
        tool.execute({ oid: hostile }, makeExec(repo.dir)),
      ).rejects.toThrowError(GitToolError);
    }
    await expect(
      tool.execute({ oid: "deadbee" }, makeExec(repo.dir)),
    ).rejects.toThrowError(GitToolError);
  });
