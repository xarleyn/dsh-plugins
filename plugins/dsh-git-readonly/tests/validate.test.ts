...
describe("validateCommitOid", () => {
  it("accepts abbreviated and full hexadecimal ids", () => {
    ...
    expect(
      validateCommitOid(" 0123456789abcdef0123456789abcdef01234567 "),
...
describe("validateRepoRelativePath", () => {
  it("normalizes separators and collapses duplicate slashes", () => {
    expect(validateRepoRelativePath("src\\lib\\util.ts", "path")).toBe(
      "src/lib/util.ts",
    );
    expect(validateRepoRelativePath("src//lib/x.ts", "path")).toBe(
      "src/lib/x.ts",
    );
  });
  ...
  it("rejects absolute paths, traversal and option look-alikes", () => {
    for (const hostile of [
      ...
      expect(() => validateRepoRelativePath(hostile, "path")).toThrowError(
        GitToolError,
      );
    }
  });
  ...
  it("treats absent input per the required flag", () => {
    ...
    expect(() =>
      validateRepoRelativePath(undefined, "file", { required: true }),
    ).toThrowError(GitToolError);
    expect(() =>
      validateRepoRelativePath(".", "file", { required: true }),
    ).toThrowError(GitToolError);
  });

  it("enforces the length cap", () => {
    expect(() =>
      validateRepoRelativePath("a".repeat(2000), "path"),
    ).toThrowError(GitToolError);
  });
});
...
describe("validateSearchLiteral", () => {
  ...
  it("rejects empty, non-string, NUL and over-long input", () => {
    ...
    expect(() => validateSearchLiteral("a\0b", "query")).toThrowError(
      GitToolError,
    );
    expect(() => validateSearchLiteral("x".repeat(300), "query")).toThrowError(
      GitToolError,
    );
  });
});
...
describe("clampLineRange", () => {
  it("defaults to a window from line 1 and flags truncation", () => {
    expect(clampLineRange(undefined, undefined, 300)).toEqual({
      from: 1,
      to: 300,
      truncated: false,
    });
    expect(clampLineRange(5, undefined, 10)).toEqual({
      from: 5,
      to: 14,
      truncated: false,
    });
  ...
  it("clamps over-long explicit spans", () => {
    expect(clampLineRange(1, 5_000, 300)).toEqual({
      from: 1,
      to: 300,
      truncated: true,
    });
  ...
  it("rejects inverted and non-positive ranges", () => {
    ...
    expect(() => clampLineRange(Number.NaN, undefined, 300)).toThrowError(
      GitToolError,
    );
  });
});
