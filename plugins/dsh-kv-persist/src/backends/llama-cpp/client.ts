...
import type {
  LlamaCppEraseResponse,
  LlamaCppRestoreResponse,
  LlamaCppSaveResponse,
...
export function normalizeSlotsResponse(
  payload: unknown,
): LlamaCppSlotsResponse {
  const rawSlots: unknown = Array.isArray(payload)
    ? payload
    : isRecord(payload)
      ? payload.slots
      : undefined;
...
export class LlamaCppClient {
  ...
  async restoreSlot(
    ...
    snapshotFilename: string,
    timeoutMs: number = this.#requestTimeoutMs,
  ): Promise<LlamaCppRestoreResponse> {
    const { status, body } = await this.#request(
  ...
  async eraseSlot(
    ...
    if (status !== 200 || !validPayload) {
      throw new KvEraseFailedError(
        `erase of slot ${slotId} failed (HTTP ${status}): ${body.slice(0, 200)}`,
      );
    }
    return { success: true };
  }
