...
export function extractJsonPayload(text: string): unknown {
  ...
  if (start < 0 || end <= start) {
    throw new SafetyGateError(
      ...
      "classifier answer contains no JSON object",
    );
  }
  let parsed: unknown;
  try {
    ...
    throw new SafetyGateError(
      "SAFETY_CLASSIFIER_INVALID_RESPONSE",
      "classifier answer is not valid JSON",
      { cause: error },
    );
  }
  return parsed;
...
export function validateVerdict(
  ...
  if (typeof decision !== "string" || !VALID_DECISIONS.has(decision)) {
    throw new SafetyGateError(
      "SAFETY_CLASSIFIER_INVALID_RESPONSE",
      `verdict decision must be one of allow|warn|review|block (got ${JSON.stringify(decision)})`,
    );
  }
  ...
  if (
    ...
    throw new SafetyGateError(
      "SAFETY_CLASSIFIER_INVALID_RESPONSE",
      "verdict confidence must be a number in [0, 1]",
    );
  }
  ...
  if (
    ...
    throw new SafetyGateError(
      "SAFETY_CLASSIFIER_INVALID_RESPONSE",
      "verdict categories must be an array of at most 16 strings",
    );
  }
  for (const category of categories as string[]) {
    if (!KNOWN_CATEGORIES.has(category)) {
      throw new SafetyGateError(
        "SAFETY_CLASSIFIER_INVALID_RESPONSE",
        `verdict category "${category}" is not part of the schema`,
      );
    }
  }
  ...
  if (typeof summary !== "string" || summary.length > maxSummaryChars) {
    throw new SafetyGateError(
      "SAFETY_CLASSIFIER_INVALID_RESPONSE",
      `verdict summary must be a string of at most ${maxSummaryChars} characters`,
    );
  }
  ...
  if (raw.policyRuleIds !== undefined) {
    if (
      !Array.isArray(raw.policyRuleIds) ||
      raw.policyRuleIds.some((id) => typeof id !== "string")
    ) {
      throw new SafetyGateError(
        "SAFETY_CLASSIFIER_INVALID_RESPONSE",
        "verdict policyRuleIds must be an array of strings",
      );
    }
  ...
  if (DECISION_ORDER[finalDecision] < DECISION_ORDER.allow) {
    throw new SafetyGateError(
      "SAFETY_CLASSIFIER_INVALID_RESPONSE",
      "verdict decision is out of range",
    );
  }
