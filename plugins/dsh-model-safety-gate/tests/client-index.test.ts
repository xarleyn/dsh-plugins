...
describe("client activation", () => {
  ...
  it("mounts the Remote and registers the card against the injected namespace", async () => {
    ...
    Object.defineProperty(remote, "safetyGate", {
      get() {
        throw new Error('cannot get property "remote.safetyGate" without inject');
      },
    });
    ...
    const face = cardFace?.() as { scope: unknown; inspect(): Promise<unknown> };
    ...
    expect(readyCtx.settingsScope.bind).toHaveBeenCalledWith({ namespace: "model-safety-gate" });
