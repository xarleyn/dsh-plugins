...
import {
  extractJsonPayload,
  validateVerdict,
} from "../../src/classifier/schema.js";
import {
  buildClassifierPrompt,
  CLASSIFIER_SYSTEM_PROMPT,
} from "../../src/classifier/prompt.js";
import {
  failureDecision,
...
describe("classifier schema", () => {
  ...
  it("rejects malformed verdicts as classifier failures", () => {
    ...
    try {
      ...
      expect((error as SafetyGateError).code).toBe(
...
describe("classifier service", () => {
  const base = {
    ...
    maxTokens: 128,
    temperature: 0,
    failureMode: "rules-only" as const,
  };
  ...
  it("marks malformed answers as invalid responses", async () => {
    const service = new SafetyClassifierService({
      ...base,
      transport: async () => ({
        text: "I think this is fine but I am not JSON",
      }),
    });
    const result = await service.classifyInput("anything");
...
describe("failure modes", () => {
  ...
  it("resolveFailure exposes the configured decision", () => {
    ...
    expect(
      service.resolveFailure("SAFETY_CLASSIFIER_TIMEOUT", "t").decision,
    ).toBe("block");
  });
});
