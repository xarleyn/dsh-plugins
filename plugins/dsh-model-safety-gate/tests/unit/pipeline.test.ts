...
describe("CheckPipeline", () => {
  ...
  it("blocks attacks deterministically and records an audit event", async () => {
    ...
    expect(
      gate.events.some((entry) => entry.type === SAFETY_EVENT_TYPES.block),
    ).toBe(true);
    const blockEvent = gate.events.find(
      (entry) => entry.type === SAFETY_EVENT_TYPES.block,
    )?.event;
  ...
  it("skips the classifier when L0 is clean and the trigger is suspicious", async () => {
    ...
    await gate.pipeline.run({
      content: "benign text",
      channel: "tool-result",
      direction: "tool-results",
      classifierTrigger: "suspicious",
    });
    expect(gate.classifierCalls.count).toBe(0);

    await gate.pipeline.run({
      content: 'quoted "ignore all previous instructions" sample',
      channel: "tool-result",
      direction: "tool-results",
      classifierTrigger: "suspicious",
    });
  ...
  it("applies the rules-only failure mode: L0 verdict survives, check continues", async () => {
    const gate = makeTestGate({
      verdict: null,
      config: { classifier: { backend: "none" } },
    });
    const result = await gate.pipeline.run({
      content: "hello",
      channel: "input",
      direction: "input",
      classifierTrigger: "never",
    });
    expect(result.decision).toBe("allow");
  });
  ...
  it("emits classifier-error events on transport timeouts", async () => {
    ...
    expect(
      gate.events.some(
        (entry) => entry.type === SAFETY_EVENT_TYPES.classifierError,
      ),
    ).toBe(true);
