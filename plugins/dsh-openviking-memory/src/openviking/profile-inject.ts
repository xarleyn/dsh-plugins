 ...
 * (not imported from the recall module) because upstream deliberately keeps
 * this module free of that module's closure — see resolveUserSpace below.
 */
type FetchJSON = (
  path: string,
  init?: { readonly method?: string; readonly body?: string },
  options?: { readonly actorPeerId?: string },
) => Promise<FetchJSONResult>;

/** The envelope fields this module reads: `ok` and the raw `result` payload. */
...
/** One flattened `.md` leaf: its display name plus the server's abstract. */
interface MemoryLeaf {
  readonly name: string;
  readonly abstract: string;
}

/** One directory listing, already fitted to its token budget. */
...
/** The assembled profile payload plus the counts callers log. */
 ...
 * narrowing step between the transport's opaque `result` and the directory
...
function elideProfile(content: string, maxTokens: number): string {
  ...
  const fallbackHeadTruncate = (): string =>
