...
/**
 * The slice of plugin config this compressor reads. Declared locally because
 * the shared module never imports the plugin's config type; the index signature
 * keeps every other key readable to the callers that thread the whole config
 * through.
 */
export interface RecallCompressConfig {
  readonly recallCompressMinInputChars?: number;
  readonly recallCompressMaxInputChars?: number;
  readonly recallCompressMaxBullets?: number;
}

/**
 * A recalled entry, read only for its `uri`: the cache key hashes the citation
 * set and the URI repair below snaps citations back onto it.
 */
interface RecallCompressEntry {
  readonly uri?: unknown;
  readonly [key: string]: unknown;
}

/** The digest cache file on disk, as `readCache` returns it. */
interface RecallDigestCache {
  readonly key?: unknown;
  readonly digest?: unknown;
  readonly updatedAt?: unknown;
}

/** Runs the digest prompt through a model; the answer is normalized downstream. */
type RunCompressor = (prompt: string) => Promise<unknown>;
...
function nearestUri(candidate: string, validUris: readonly string[]): string {
  let best = "";
...
export async function compressRecallContext({ query, rendered, shortContext = rendered, entries = [], cfg = {}, runCompressor, cachePath = "", now = 0 }: {
  ...
  entries?: readonly RecallCompressEntry[];
  cfg?: RecallCompressConfig;
  runCompressor: RunCompressor;
  cachePath?: string;
  now?: number;
}): Promise<{ status: string; context: string }> {
