 ...
 * their own template.
 */
export const PEER_SOURCE_PRESETS: Record<"git" | "cwd" | "none", string[]> = {
 ...
 */
export function peerSourceTemplates(
  source: unknown,
...
export interface EffectivePeer {
  ...
  readonly source: "explicit" | "none" | "workspace";
  readonly origin: string;
  readonly legacyPeerId: string;
}

/**
 * Caller-supplied settings, kept `unknown` because they cross the plugin's
 * config boundary: every field is coerced here exactly as the original did,
 * and a supplied `identity` is one this module already produced.
 */
export interface EffectivePeerOptions {
  readonly cfg?: {
    readonly peerId?: unknown;
    readonly workspacePeer?: unknown;
    readonly peerSource?: unknown;
  ...
  readonly onWarn?: ((message: string) => void) | null;
}
