...
const stagedRoot = join(workspaceRoot, "packages", "dsh-plugin-log-ui");
...
try {
  ...
  writeFileSync(join(workspaceRoot, "tsconfig.host.json"), `${JSON.stringify({
    ...
    references: [
      ...
      { path: "./packages/dsh-plugin-log-ui" },
    ],
    compilerOptions,
  }, null, 2)}\n`);
  ...
  const workspace = new WorkspaceAnalyzer({
    root: workspaceRoot,
    packages: ["@yadsh/dsh-plugin-log-ui"],
    faces: ["host"],
  }).analyze();
  const face = workspace.faces.find((candidate) => candidate.face === "host");
  const artifact = face === undefined
    ? undefined
    : new FaceModelEmitter(face).emit("@yadsh/dsh-plugin-log-ui");
  if (artifact === undefined || artifact.remote === undefined) {
    throw new Error(`Typert did not discover the pluginLogUi Remote service: ${JSON.stringify(
      face?.packages.map(({ name, services, invocations }) => ({
        name,
