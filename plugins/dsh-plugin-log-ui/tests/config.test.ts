...
import { DEFAULT_PLUGIN_LOG_UI_CONFIG, resolveConfig } from "../src/config.js";
...
describe("plugin log UI config", () => {
  ...
  it("rejects invalid plugin ids and values at runtime", () => {
    expect(() => resolveConfig({ levels: { "bad id": "info" } })).toThrow(
      "plugin log override id is invalid",
    );
    expect(() => resolveConfig({ format: "pretty" as "text" })).toThrow(
      "plugin log format is invalid",
    );
  });
});
