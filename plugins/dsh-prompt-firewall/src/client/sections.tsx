  ...
  return (
    <section className="pf-section">
      ...
      <div className="pf-grid">
        <Toggle checked={config?.enabled ?? true} disabled={!writable} label="Firewall enabled" hint="Keep auditing available when policy is off." onChange={value => { setPath(['enabled'], value) }} />
...
export function LastRequestSection(props: {
  ...
  const effectiveMode = props.inspector?.config.mode ?? props.config?.mode ?? 'blocklist'
  return (
    <section className="pf-section">
      <div className="pf-section-title"><h3>Last request</h3><button className="pf-btn" disabled={props.refreshing} onClick={() => { props.onRefresh() }}>{props.refreshing ? 'Refreshing…' : 'Refresh'}</button></div>
      <div className="pf-stats">
        ...
        <div className="pf-stat"><b>{last === null ? '—' : '~' + String(last.estimatedTokensRemoved)}</b><span>tokens removed (estimate)</span></div>
  ...
  return (
    <section className="pf-section">
      ...
      <form className="pf-editor" onSubmit={addRule}>
        ...
        <select className="pf-control" value={ruleKind} disabled={!writable || ruleAction === 'protect'} onChange={event => { setRuleKind(event.currentTarget.value as RuleKind) }}>
        ...
        {last === null ? <div className="pf-empty">No audited prompt assembly yet.</div> : <table className="pf-table">
          ...
          <tbody>{last.sections.map((section, index) => <tr key={`${section.name}:${index}`}>
            ...
            <td><div className="pf-actions">
              <button className="pf-btn link" disabled={!props.writable} onClick={() => { void props.setPolicy(section.name, 'allow') }}>Allow</button>
              <button className="pf-btn link danger" disabled={!props.writable} onClick={() => { void props.setPolicy(section.name, 'block') }}>Block</button>
              <button className="pf-btn link" disabled={!props.writable} onClick={() => { void props.setPolicy(section.name, 'protect') }}>Protect</button>
              <button className="pf-btn link" disabled={!props.writable} onClick={() => { void props.setPolicy(section.name, 'clear') }}>Clear</button>
            </div></td>
          </tr>)}</tbody>
        </table>}
      </div>
