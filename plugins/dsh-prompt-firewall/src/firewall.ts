...
function validateAssembly(
...
): asserts assembly is FirewallAssembly {
  if (!Array.isArray(assembly.sections))
    throw new TypeError("prompt assembly sections must be an array");
  for (const section of assembly.sections) {
    ...
    if (typeof section.text !== "string") {
      throw new TypeError(
        `prompt section "${section.name}" text must be a string`,
      );
    }
  }
}
      ...
      safeLog(
        ...
        `[prompt-firewall] BLOCK ${section.name}: ${section.reason}, ` +
          ...
          (section.preview === undefined
            ? ""
            : `, preview=${JSON.stringify(section.preview)}`),
      );
...
export function applyFirewall(
  ...
  try {
    ...
    const rules =
      options.evaluate === undefined ? compileRules(config) : undefined;
    const evaluator =
      options.evaluate ?? ((section) => evaluateSection(section, rules!));
    ...
    if (
      ...
      return assembly;
    if (!decisions.some((decision) => decision.action === "block"))
      return assembly;

    return {
      ...assembly,
      sections: assembly.sections.filter(
        (_, index) => decisions[index]?.action !== "block",
      ),
    };
  } catch (error) {
    try {
