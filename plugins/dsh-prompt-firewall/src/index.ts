...
export class PromptFirewall
  ...
  implements PromptFirewallService
{
  ...
  async setSectionPolicy(
    ...
    if (name.length === 0)
      throw new TypeError("section name must be non-empty");
    const settings = this.ctx.get("settings");
    if (settings === undefined) {
      throw new Error(
        "prompt-firewall settings are unavailable; mount a DSH settings provider",
      );
    }
