...
describe("applyFirewall", () => {
  ...
  it("logs preview only when explicitly enabled", () => {
    const warn = vi.fn();
    applyFirewall(
      assembly([{ name: "plugin:noisy", text: "sensitive value" }]),
      {
  ...
  it("fails open and logs an internal evaluator error", () => {
    ...
    expect(error).toHaveBeenCalledWith(
      expect.stringContaining("forced failure"),
    );
  });
  ...
  it("still filters when routine audit logging fails", () => {
    const result = applyFirewall(
      assembly([{ name: "plugin:noisy", text: "text" }]),
      {
        config: resolveConfig({ blockedSections: ["plugin:noisy"] }),
        logger: {
          info: () => {
  ...
  it("bounds audit history while retaining known section names", () => {
    ...
    applyFirewall(assembly([{ name: "plugin:first", text: "one" }]), {
      config,
      auditStore: store,
    });
    applyFirewall(assembly([{ name: "plugin:second", text: "two" }]), {
      config,
      auditStore: store,
    });

    expect(store.entries()).toHaveLength(1);
    expect(store.knownSections().map((section) => section.name)).toEqual([
      "plugin:first",
      "plugin:second",
    ]);
  });
