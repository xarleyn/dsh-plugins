...
describe("DSH system-prompt integration", () => {
  it("filters sections added by contributors while preserving core and tool guidance", async () => {
    ...
    expect(
      remoteMethods(ctx.promptFirewall).map(
        (method) => method.exportName ?? method.method,
      ),
  ...
  it("post-processes sections added by later waterfall listeners", async () => {
    ...
    const result = await ctx.systemPrompt.assemble();
    expect(result.sections.map((section) => section.name)).not.toContain(
      "plugin:late:announcement",
    );
  });
  ...
  it("lets audit-only and strict presets supply their mode defaults", async () => {
    ...
    auditContext.systemPrompt.section({
      name: "plugin:visible",
      order: 10,
      text: "visible",
    });
    expect(
      (await auditContext.systemPrompt.assemble()).sections.map(
    ...
    strictContext.systemPrompt.section({
      name: "plugin:unknown",
      order: 10,
      text: "unknown",
    });
    expect(
      (await strictContext.systemPrompt.assemble()).sections.map(
        (section) => section.name,
      ),
