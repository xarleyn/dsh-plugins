...
describe("rule engine", () => {
  it("blocks an exact section", () => {
    expect(
      evaluate("plugin:dsh-ssh", { blockedSections: ["plugin:dsh-ssh"] }),
    ).toEqual({
      action: "block",
      reason: "exact-block",
      rule: "plugin:dsh-ssh",
  ...
  it("blocks a matching prefix", () => {
    expect(
      evaluate("plugin:advertisement:foo", {
        blockedPrefixes: ["plugin:advertisement:"],
      }),
    ).toEqual({
      action: "block",
      reason: "prefix-block",
      rule: "plugin:advertisement:",
    });
  });

  it("protects verified core namespaces before block rules", () => {
    expect(evaluate("tool:run_code", { blockedPrefixes: ["tool:"] })).toEqual({
      action: "protect",
      reason: "core-protect",
      rule: "tool:",
    });
    expect(evaluate("tools:sdk", { blockedPatterns: ["tools:*"] })).toEqual({
      action: "protect",
      reason: "core-protect",
      rule: "tools:",
    });
  ...
  it("honors the specified exact/prefix/pattern priority", () => {
    expect(
      evaluate("plugin:foo:announcement", {
  ...
  it("allows unknown sections in blocklist mode", () => {
    expect(evaluate("plugin:unknown")).toEqual({
      action: "allow",
      reason: "blocklist-default",
    });
  });

  it("blocks unknown sections in allowlist mode unless protected", () => {
    expect(evaluate("plugin:unknown", { mode: "allowlist" })).toEqual({
      action: "block",
      reason: "allowlist-default",
    });
    expect(evaluate("harness:identity", { mode: "allowlist" }).action).toBe(
      "protect",
    );
  });
  ...
  it("applies unknownPluginPolicy only to plugin-owned sections", () => {
    expect(
      evaluate("plugin:unknown", { unknownPluginPolicy: "block" }).reason,
    ).toBe("unknown-plugin-policy");
    expect(
      evaluate("workspace:rules", { unknownPluginPolicy: "block" }).action,
    ).toBe("allow");
  });

  it("never blocks in audit mode", () => {
    expect(
      evaluate("plugin:blocked", {
        mode: "audit",
