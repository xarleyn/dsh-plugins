...
Развёртывание может завести один read-only сервисный токен и разрешить подключениям работать от его имени: выпустить personal access token может не каждый — у подрядчика нет своей учётной записи в корпоративном GitLab, стажёру не выдали прав на токен, — а один централизованно сменяемый аккаунт снимает с пользователей и ротацию, и вопрос «чей это был доступ». Владелец токена — развёртывание: пользователь не может ни создать профиль, ни сменить секрет, ни выбрать профиль по id. Возможность реализована у GitLab и TeamCity; остальные интеграции сервисного режима не предлагают, и в их карточках о нём ничего не появляется.
...
Сервисный режим — не «read-only вообще», а жёсткий потолок: только `read`, только обычная чувствительность и только операции, которые провайдер пометил service-safe намеренно. Запись, админ-действия, управление правами и токенами и произвольный REST недостижимы, даже если сервисный токен наверху это умеет: классификация операции — свойство каталога провайдера, а не прав токена. Чувствительные чтения остаются личными: через общий аккаунт не читаются лог CI-джоба GitLab (`ci.logs.read`), лог сборки TeamCity (`logs.read`) и текст артефакта (`artifacts.read`), а конфиденциальная задача GitLab не возвращается вовсе — прямое чтение отвечает `ResourceNotFound`, из выдачи и из поиска она выпадает, а поиск по комментариям (`scope: notes`) отказывается целиком: у комментария нет признака конфиденциальности, и проверить его родителя одним запросом нельзя. Операция, которую провайдер не классифицировал, отклоняется по умолчанию: инструмент, добавленный обновлением провайдера, недостижим через общий аккаунт, пока кто-то не классифицирует его намеренно. Администраторская `policy` может только выключить: `allow` в профиле отвергается при загрузке, потому что оператор, написавший `allow`, считает, что расширил потолок.

Граница ресурсов нужна потому, что сервисный аккаунт видит больше, чем один пользователь. В сервисном режиме провайдер сверяет с allowlist каждый вызов: проект, названный по id или по пути, должен быть в списке или под перечисленной группой; сборка TeamCity, названная по id, сначала разрешается в свой проект; листинг, не называющий проект, отказывается, а не отвечает всей картиной, доступной общему аккаунту. Список проектов (`gitlab_projects_list`, `teamcity_projects`) собирается из allowlist, а не из того, что видит сервисный токен: проект, которого токен не видит, приходит в `unavailableResources`, а не исчезает молча, — это и есть ответ оператору, что граница и токен разошлись. Разворачивая группу GitLab, провайдер просит только её собственные проекты (`with_shared=false`) и дополнительно сверяет каждый вернувшийся проект с границей: чужой проект, расшаренный в группу, в выдачу не попадает. Пользователь может сузить список в карточке кнопкой «Изменить доступ», но не расширить: выбор вне администраторского списка отбрасывается и не сохраняется.
