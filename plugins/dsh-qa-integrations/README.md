...
6. адрес внешнего сервиса приходит только из конфига оператора: у GitLab, Jira, Confluence и Test IT это выбранная инсталляция из списка, объявленного оператором, у Bitrix24 — проверенный по доменному allowlist URL вебхука, у TeamCity — адрес, проверенный по политике стенда (и при сохранении, и на каждом вызове);
...
Unit/integration набор проверяет AEAD, неверный ключ, rewrap, SSRF-ограничение webhook URL, адресов инстансов и политики адресов TeamCity, redaction (включая форму gitlab-токена и секреты, напечатанные в логе), escape locator-значений, нарезку и чистку логов, отказ по бинарным и архивным артефактам, отсутствие model-visible identity/secret selectors, отказ unowned/subagent-сессии, соответствие каждого tool операции каталога, read-only характер всех четырёх каталогов, сужение возможностей по scopes вебхука и токена, отображение ошибок GitLab, TeamCity и Confluence в доменные коды, обрезку больших ответов и параллельную изоляцию Alice/Bob. Для Confluence отдельно проверяются сборка CQL с экранированием литералов, разбор Atlassian Document Format с плейсхолдерами для макросов и медиа, отказ в пространстве вне allowlist и на поиске, и на прямом чтении, вложение `untrustedContent` в ответы со страницами и комментариями, а также отказ токену, чей сайт убрали из конфига. Для Test IT отдельно проверяются канонизация списка инсталляций и отказ форме подключения без выбранной инсталляции, проба подключения документированным запросом, только-GET каталог и запрет путей с POST-поисками, зажим лимитов и конверты страниц, отказ бинарным и слишком большим вложениям до скачивания байтов, redaction секрета, напечатанного в текстовом вложении, и то, что токен не попадает ни в URL, ни в тело. Отдельный гейт следит за границей провайдеров: `verify:package` падает, если в общем слое появится имя конкретной интеграции, а тест `provider boundary` — если брокер начнёт зависеть от Bitrix24. Собранный клиентский бандл проверяется отдельным тестом: он поднимается в песочнице как в браузере — через `window.__ModuleLoader__` с таблицей модулей, в которой есть только React, — регистрируется под полным именем пакета и монтирует карточку Confluence из секции, которую сам зарегистрировал.
