...
- **Rate limiting** (§19): в личном режиме лимиты запросов в минуту и на сервер не считаются; вместо них — ограниченные размеры страниц и повторы с backoff для 429/5xx.
...
- **Rate limiting** (раздел «Rate limiting»): в личном режиме лимиты по principal, ресурсу и глобальные не считаются; вместо них — ограниченные размеры ответов и повторы с backoff для 429/5xx.
...
- **Кэш** (раздел «Caching») отсутствует полностью, поэтому и межпользовательской утечки через кэш быть не может; пагинация stateless, подписанных continuation-токенов нет.
- **Rate limiting** (раздел «Rate limiting»): в личном режиме лимитов на аккаунт и на инстанс нет; вместо них ограниченные страницы и текст, повторы с backoff (включая `Retry-After`, если он числовой) и полный отказ от повторов на 4xx.
...
- **Rate limiting** (§Rate limiting): в личном режиме лимитов на пользователя и на сайт нет; вместо них — ограниченные страницы, таймаут стенда (`timeoutMs`) и повторы с `Retry-After` для 429/5xx. Инвалидация по webhooks (§Future work) — тоже не подключена.
...
managedServiceCredentials:
  ...
  rateLimit: # потолок сервисного режима; 0 отключает измерение
    perPrincipal:
...
Rate limiting сервисного режима считается по обеим сторонам сразу: `rateLimit.perPrincipal.requestsPerMinute` — сколько запросов в минуту одному пользователю на провайдера, `rateLimit.perCredential.requestsPerMinute` — сколько выдерживает общий токен, и `perCredential.maxConcurrent` — сколько вызовов на нём разрешено держать в полёте. Пороги расходуются непрерывно, а не сбрасываются по минуте: всплеск съедает минуту вперёд, дальше идёт по равномерной скорости. Проверки читаются до списания, поэтому отказ всегда по одному основанию: вызов, которому не хватило общего бюджета, не тратит личный, и наоборот — иначе пользователь, упирающийся в общий потолок, платил бы за него собственным будущим бюджетом. Личный режим не ограничивается: частоту там задаёт сам сервер. Отказ отвечает `RateLimited`, в аудите записывается как `denied` с реальным пользователем и наверх не уходит; лимит живёт в этом процессе, где стоит единственный путь к сервисному токену. `0` отключает измерение. Межвызовных кэшей результатов в плагине нет ни у одного провайдера, поэтому переключение режимов не может отдать устаревшие данные.
...
`managedServiceCredentials` — срез, которого нет, пока `enabled: false` (значение по умолчанию): `defaultForNewConnections` решает, начинается ли новое подключение на сервисном токене, `rateLimit` задаёт потолок частоты сервисного режима (`perPrincipal.requestsPerMinute`, `perCredential.requestsPerMinute` и `perCredential.maxConcurrent`; значения по умолчанию — 120, 1000 и 16, `0` отключает измерение, дробное или отрицательное число отвергается при загрузке), а каждый профиль связывает провайдера и его инстанс с секретом общего аккаунта (`secretFile` или `secretEnv`, ровно один из двух) и обязательной границей `resources`, внутри которой `policy` может только выключать операции. Провайдер и инстанс в профиле должны существовать в этом же конфиге, иначе загрузка падает, а сообщение называет профиль или инстанс. Правила и границы режима — в разделе «Сервисные токены (managed service credentials)».
