...
| `jira_search_issues` | Поиск задач по тексту, проектам, статусам, автору, исполнителю, меткам и датам |
| `jira_get_issue` | Карточка задачи: ключ, ссылка, проект, тип, статус, приоритет, люди, метки, даты и описание |
...
Описания и комментарии приходят в Atlassian Document Format и рендерятся в текст с сохранением структуры: заголовки, списки, код, ссылки, упоминания, таблицы и вложения-маркеры. Сырой JSON документа модели не отдаётся, а встроенные ссылки и медиа-узлы не загружаются. Кастомные поля называются по схеме сайта (`jira_get_fields` показывает её модель целиком): схема читается только после того, как задача ответила, и **не кэшируется** — один и тот же сайт отдаёт разный список полей двум пользователям с разными правами, а кэш на двоих был бы утечкой ради одного запроса. Тело задачи, комментария или кастомного поля ограничено `maxTextChars` (по умолчанию 20 000 символов); при обрезке ответ помечается `descriptionTruncated` или `bodyTruncated`. Текст из Jira — данные, а не инструкция.
...
| `issues.read` | Поиск задач и карточка задачи с описанием, связями и кастомными полями |
...
| `fields.read` | Схема полей сайта, включая кастомные |
...
- **История изменений задачи** (changelog) не читается, хотя спецификация упоминает её в `include`. Чтение кастомных полей идёт селектором `*all`, поэтому на сайте с очень большим числом полей такой ответ может упереться в `ResultTooLarge`.
