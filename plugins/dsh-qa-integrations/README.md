...
Один каталог — одна интеграция. Всё, что знает про Bitrix24, лежит в `src/providers/bitrix24/`, про GitLab — в `src/providers/gitlab/`, про TeamCity — в `src/providers/teamcity/`, про Confluence — в `src/providers/confluence/`: каталог возможностей и операций, построение запросов, HTTP-граница, срез конфига и тулы. У TeamCity рядом лежат разбор locator-синтаксиса (`locators.ts`), чистка и нарезка логов (`logs.ts`), политика артефактов (`artifacts.ts`) и разбор адресов стенда (`network.ts`) — это те части, которые иначе расползлись бы по `operations.ts`. У Confluence так же вынесены две: `adf.ts` превращает Atlassian Document Format в markdown-подобный текст, а `cql.ts` собирает CQL из типизированных фильтров и экранирует литералы. Общий слой — брокер, репозиторий, секреты, обвязка тулов — не знает ни одной интеграции по имени: capability это свободная строка, а `providers/contract.ts` описывает, что должен уметь провайдер. Порядок добавления следующей интеграции (Confluence, …) и правила, которые проверяет гейт пакета, — в [src/providers/README.md](https://github.com/xarleyn/dsh-plugins/blob/main/plugins/dsh-qa-integrations/src/providers/README.md).
...
6. адрес внешнего сервиса приходит только из конфига оператора: у GitLab и Confluence это выбранный сайт из списка инстансов, у Bitrix24 — проверенный по доменному allowlist URL вебхука, у TeamCity — введённый пользователем адрес, проверенный по политике адресов стенда (и при сохранении, и на каждом вызове).
