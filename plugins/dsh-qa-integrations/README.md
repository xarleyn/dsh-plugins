...
Персональные интеграции для `@yadsh/dsh-qa-surface`. Плагин подключает Bitrix24 через URL входящего вебхука, GitLab через personal access token и TeamCity через access token, после чего даёт агенту набор read-only инструментов, ограниченный и правами подключения, и персональной политикой пользователя.
...
Один каталог — одна интеграция. Всё, что знает про Bitrix24, лежит в `src/providers/bitrix24/`, про GitLab — в `src/providers/gitlab/`, про TeamCity — в `src/providers/teamcity/`: каталог возможностей и операций, построение запросов, HTTP-граница, срез конфига и тулы. У TeamCity рядом лежат разбор locator-синтаксиса (`locators.ts`), чистка и нарезка логов (`logs.ts`), политика артефактов (`artifacts.ts`) и разбор адресов стенда (`network.ts`) — это те части, которые иначе расползлись бы по `operations.ts`. Общий слой — брокер, репозиторий, секреты, обвязка тулов — не знает ни одной интеграции по имени: capability это свободная строка, а `providers/contract.ts` описывает, что должен уметь провайдер. Порядок добавления следующей интеграции (TeamCity, …) и правила, которые проверяет гейт пакета, — в [src/providers/README.md](https://github.com/xarleyn/dsh-plugins/blob/main/plugins/dsh-qa-integrations/src/providers/README.md).
...
Подключение — персональный access token TeamCity. Возможность появляется у пользователя, если её включил оператор (`teamcity.*Read`).
...
6. адрес внешнего сервиса приходит только из конфига оператора: у GitLab это выбранный инстанс, у Bitrix24 — проверенный по доменному allowlist URL вебхука, у TeamCity — введённый пользователем адрес, проверенный по политике адресов стенда (и при сохранении, и на каждом вызове).
