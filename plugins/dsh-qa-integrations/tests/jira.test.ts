...
describe("jql builder", () => {
  ...
  it("builds the filter set the specification describes", () => {
    expect(
      ...
      'project in ("MDC", "PLATFORM") AND status in ("In Progress") AND labels = "regression" AND labels = "qa" AND assignee = currentUser() AND reporter = "5b10ac8d82e05b22cc7d4ef5" AND created >= "2026-09-01 10:30" AND updated >= "2026-09-01" ORDER BY updated DESC',
    );
  });
...
describe("jira issue history and lists", () => {
  const ISSUE = {
    ...
    fields: {
      summary: "Payment timeout",
      components: [{ id: "1", name: "MDC. Техдолг" }],
  ...
  it("answers the compact card with components and versions", async () => {
    ...
    expect(item?.["components"]).toEqual(["MDC. Техдолг"]);
  ...
  it("adds the change history only when it was asked for", async () => {
    ...
    const withHistory = (await provider.execute(
      { credential: credentialFor(provider) },
      "issues.get",
      { issueKey: "MDC-123", include: ["changelog_summary"] },
    )) as Record<string, unknown>;
