...
source-priority notes — is editable without touching the source: the «Заметки
модели» section of the settings card (the `notes` config block) mutes each note
and rewords its text, keeping the generated parts (`{identity}`,
`{instructions}`, `{reportTool}`) as placeholders. The source-priority note is
the one that says an answer belongs to the documentation or the expert before
