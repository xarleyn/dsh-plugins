...
## 10.1 Deleting a conversation

Кнопка «Удалить разговор» есть только у роли Admin (permission
`conversations.delete`) и только на странице разговора. Она удаляет разговор
со стенда по-настоящему, а не прячет строку:

- **журналы сессий** — сам чат и все сессии, делегированные из него
  (транзитивно: субагентская сессия — часть ответа, внутри которого она
  работала), — удаляются из хранилища стенда. Харнесс шва удаления не имеет:
  `sessionPersistence` умеет create/open/flush/stat/list, а живая сессия
  исчезает из памяти только вместе с владеющим ей fiber. Поэтому удаление
  снимает артефакты хранилища — каталоги сессий под `$DSH_HOME/sessions`, — и
  после этого сессия уходит и из `session/list`;
- **запись владения** (вместе с активациями навыков и снимками политики) —
  иначе первый обратившийся забрал бы осиротевшую сессию;
- **записи качества** — оценки, вердикты и очередь по этому разговору;
- **источники** — собранные разговором шарды провенанса.

Действие пишется в аудит (`conversation.deleted`) с владельцем и числом
удалённых сессий: это уничтожение того, что написал человек, а не скрытие
строки.

Отказы приходят **до** любых изменений, поэтому разговор никогда не удаляется
наполовину:

| Отказ | Когда |
|---|---|
| `conversation-unknown` | разговора нет ни в списке сессий, ни в записях владения |
| `conversation-live` | харнесс держит сессию в памяти: её журнал вернётся следующим flush |
| `conversation-not-removable` | стенд не умеет удалять хранимые разговоры (нет шва удаления, либо журналы лежат не файлами) |

Удаление в **сайдбаре** (кнопка «Удалить чат») остаётся прежним и ничего
серверного не делает: оно убирает строку только в этом браузере. Это разные
действия, и подтверждение удаления в админке говорит об этом прямо.
