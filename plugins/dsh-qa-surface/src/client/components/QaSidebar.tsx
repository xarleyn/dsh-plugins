...
export function buildChatRows(
  ...
  for (const id of chatIds) {
    ...
    rows.push({
      id,
      title: summary.blank ? "New chat" : summary.displayTitle,
...
export function QaSidebar(props: QaSidebarProps) {
  return (
    <nav className="dsh-qa-sidebar" aria-label="Chat history">
      <div className="dsh-qa-sidebar__head">
        <span className="dsh-qa-sidebar__title">Chats</span>
        {props.showNewChat ? (
          <button
            type="button"
      ...
      <div className="dsh-qa-sidebar__list">
        {props.rows.length === 0 ? (
          <p className="dsh-qa-sidebar__empty">No chats yet</p>
        ) : (
          props.rows.map((row) => (
            <button
              key={row.id}
