...
.qa-card-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:11px}
.qa-card-field{display:grid;gap:6px}
.qa-card-field>span{font-size:11px;color:var(--dsw-alias-label-secondary);font-weight:600}
.qa-card-control{width:100%;height:36px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-layer-3);color:var(--dsw-alias-label-primary);padding:0 10px;font:inherit;font-size:12px;outline:none}
textarea.qa-card-control{height:auto;padding:8px 10px;line-height:1.5;resize:vertical}
...
.qa-card-toggle-row{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 12px;border:1px solid var(--dsw-alias-border-l2);border-radius:9px}
.qa-card-toggle-copy{display:grid;gap:2px}
.qa-card-toggle-copy strong{font-size:12px;font-weight:500}
.qa-card-toggle-copy span{font-size:10px;color:var(--dsw-alias-label-tertiary)}
...
@media(max-width:720px){.qa-card-grid{grid-template-columns:1fr}}
`;
