 ...
 * {@link DEFAULT_QA_DOCUMENTS_CONFIG} so an untouched deployment sends the
...
export function resolveDocumentsConfig(
  input: QaDocumentsConfig = {},
): ResolvedQaDocumentsConfig {
  const D = DEFAULT_QA_DOCUMENTS_CONFIG;
...
export function applyDocumentsEnvOverrides(
  ...
  const storageRoot = value("QA_DOCUMENTS_STORAGE_ROOT");
  const templatesRoot = value("QA_DOCUMENTS_TEMPLATES_ROOT");
  const doclingBaseUrl = value("QA_DOCLING_BASE_URL");
  const doclingTimeout = value("QA_DOCLING_TIMEOUT_MS");
  const pandocExecutable = value("QA_PANDOC_EXECUTABLE");
  const libreofficeExecutable = value("QA_LIBREOFFICE_EXECUTABLE");
  const markitdownExecutable = value("QA_MARKITDOWN_EXECUTABLE");
  const ocrLanguages = value("QA_DOCUMENTS_OCR_LANGUAGES");
  const maxInputBytes = value("QA_DOCUMENTS_MAX_INPUT_BYTES");
  return {
    ...config,
    enabled: envBoolean(value("QA_DOCUMENTS_ENABLED"), config.enabled ?? true),
    storage: {
      ...config.storage,
...
export function retentionIntervalMs(config: ResolvedQaDocumentsConfig): number {
