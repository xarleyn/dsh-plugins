...
describe("QaChatIndex", () => {
  ...
  it("adds new chats without reordering the existing index", () => {
    const storage = memoryStorage({
      "dsh-qa-surface.session:v1:/qa:chats": JSON.stringify(["a", "b", 42]),
    });
    expect(index(storage).chatIds()).toEqual(["a", "b"]);
    index(storage).addChat("a");
    expect(index(storage).chatIds()).toEqual(["a", "b"]);
    index(storage).addChat("c");
    expect(index(storage).chatIds()).toEqual(["c", "a", "b"]);
  });

  it("caps the index length", () => {
    const storage = memoryStorage();
    for (let i = 0; i < 60; i += 1) index(storage).addChat(`chat-${i}`);
  ...
  it("survives a throwing storage without affecting callers", () => {
    ...
    expect(() => chats.addChat("s1")).not.toThrow();
