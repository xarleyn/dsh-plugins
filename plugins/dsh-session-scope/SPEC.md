...
**Выполнено (2026-09-22).** Отдельный change состоялся: `src/client.ts` стал
обычным модулем, который собирается общим tsdown-пайплайном
(`tsdown.config.ts`) — регистрацию `window.__ModuleLoader__.load` и фабрику
печатает сборка, `lib/client.js` больше не редактируется руками. Файл исключён
из `tsc`-сборки, поэтому `./client` в манифесте указывает прямо на бандл.
Разделение этапов уважено: scope-архитектура и UI не менялись, изменение —
типизация, линт и сборка. Модульное разбиение клиента на несколько файлов
остаётся отдельным change.
