...
async function unary(method, payload) {
  ...
  if (data.result === undefined || data.result.ok !== true) {
    throw new Error(
      `${method} failed: ${JSON.stringify(data.result ?? data).slice(0, 300)}`,
    );
  }
...
console.log(
  `dumped ${events.length} events in ${pages} pages, seq ${events[0]?.seq}..${events.at(-1)?.seq}`,
);
...
for (let i = 1; i < events.length; i++) {
  if (events[i].seq !== events[i - 1].seq + 1) {
    throw new Error(
      `dump has a seq gap at ${events[i - 1].seq} -> ${events[i].seq}`,
    );
  }
}
...
if (!rawHeader)
  throw new Error(
    "WSS_HEADER environment variable with the session header JSON is required",
  );
const parsedHeader = JSON.parse(rawHeader);
if (parsedHeader.id !== sessionId)
  throw new Error(
    `WSS_HEADER id ${parsedHeader.id} does not match ${sessionId}`,
  );
const headerLine = JSON.stringify(parsedHeader) + "\n";
const CHECKSUM_OPTIONS = { params: { [constants.ZSTD_c_checksumFlag]: 1 } };
const frame = (text) =>
  zstdCompressSync(Buffer.from(text, "utf8"), CHECKSUM_OPTIONS);
...
function scanFrames(buffer) {
  ...
  while (offset < buffer.length) {
    ...
    if (buffer.readUInt32LE(offset) !== ZSTD_MAGIC)
      return { frames, error: `bad magic at ${offset}`, tornStart: start };
    offset += 4;
...
function verify(path) {
  ...
  if (error !== undefined || tornStart !== undefined)
    throw new Error(`published artifact invalid: ${error ?? "torn tail"}`);
  const firstPlain = zstdDecompressSync(
    published.subarray(frames[0].start, frames[0].end),
  ).toString("utf8");
  if (
    firstPlain.length === 0 ||
    firstPlain.indexOf("\n") !== firstPlain.length - 1
    ...
    throw new Error(
      "published artifact: first frame is not exactly one header line",
    );
  }
  let seqs = [];
  for (const fr of frames) {
    const plain = zstdDecompressSync(
      published.subarray(fr.start, fr.end),
    ).toString("utf8");
  ...
  for (let i = 1; i < seqs.length; i++) {
    if (seqs[i] !== seqs[i - 1] + 1)
      throw new Error(
        `published artifact has a seq gap at ${seqs[i - 1]} -> ${seqs[i]}`,
      );
  }
  console.log(
    `verified: ${frames.length} frames, ${seqs.length} events, header OK, contiguous 0..${seqs.at(-1)}`,
  );
  return { frames, seqs };
}
...
if (!sessionDir)
  throw new Error("WSS_SESSION_DIR environment variable is required");
