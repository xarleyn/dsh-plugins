...
try {
  ...
  if (tarball === undefined) {
    ...
    const tarballs = (await readdir(artifactDirectory)).filter((name) =>
      name.endsWith(".tgz"),
    );
    if (tarballs.length !== 1)
  ...
  if (!composed.stdout.includes("dsh-session-scope")) {
    throw new Error(
      "packed plugin is absent from the composed DSH web profile",
    );
  }
  ...
  if (!("@yadsh/dsh-session-scope" in installed)) {
    throw new Error(
      "DSH profile manifest does not contain @yadsh/dsh-session-scope",
    );
  }

  completed = true;
  console.log(
    `packed DSH ${dshVersion} composition smoke passed on ${process.platform}`,
  );
} finally {
