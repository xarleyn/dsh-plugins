...
if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(manifest.version)) {
  throw new Error(
    `package version is not SemVer: ${JSON.stringify(manifest.version)}`,
  );
}
...
for (const file of requiredFiles) {
  if (!manifest.files?.includes(file))
    throw new Error(`package files omit ${file}`);
}
...
if (manifest.exports?.["./client"]?.default !== "./lib/client.js") {
  throw new Error(
    "./client must expose the default bundle path required by DSH client composition",
  );
}
...
if (!manifest.dsh?.client?.inject?.includes("@deepseek-ai/dsh-api-gateway")) {
  throw new Error(
    "client composition must inject the API gateway for sessionScope/list",
  );
}
...
if (
  /dsh-draft-sessions|draftSessions\//.test(JSON.stringify(manifest) + patch)
) {
  throw new Error(
