...
const DENIAL_MESSAGE =
  "Path is outside the active session scope. Change the session scope if access is required.";
...
export class SessionScopeRuntime {
  ...
  run<T>(
    session: ScopeSession | undefined,
    operation: () => T,
    execution?: ScopeToolExecution,
  ): T {
    if (
      session === undefined ||
      getScope(session, this.fallbackWorkspaceRoot).mode === "full"
    ) {
      return operation();
    }
  ...
  private async resolvedScope(
    ...
    const key = JSON.stringify([
      ...
      scope.workspaceRoot,
      scope.roots,
      scope.navigationRoots,
    ]);
    const cached = this.cache.get(session);
    if (
      cached?.eventCount === session.snapshotEvents().length &&
      cached.key === key
    )
      return cached;
    const resolveOne = (path: string) =>
      fs.resolve(path, { cwd: scope.workspaceRoot });
    const value: CachedScope = {
      scope,
      workspace: await resolveOne(scope.workspaceRoot),
      workspaceSpellings: [
        scope.workspaceRoot,
        session.header?.cwd ?? fallbackWorkspaceRoot,
      ],
      content: await Promise.all(scope.roots.map(resolveOne)),
      navigation: await Promise.all(
        [...new Set([scope.workspaceRoot, ...scope.navigationRoots])].map(
          resolveOne,
        ),
      ),
  ...
  private classify(
    ...
    if (!fs.contains(resolved.workspace, target)) {
      const originatedInsideWorkspace = resolved.workspaceSpellings.some(
        (root) => root.length > 0 && isLexicallyUnder(target.displayPath, root),
      );
      return originatedInsideWorkspace ? "denied" : "content";
    }
    if (resolved.content.some((root) => fs.contains(root, target)))
      return "content";
    if (
      resolved.navigation.some(
        (root) => fs.contains(root, target) && fs.contains(target, root),
      )
    ) {
      return "navigation";
    }
  ...
  patchFileSystem(
    fs: ScopeAwareFileSystem,
    fallbackWorkspaceRoot = this.fallbackWorkspaceRoot,
  ): () => void {
    const original = {
    ...
    const resolveCurrent = async () => {
      ...
      return session === undefined
        ? undefined
        : this.resolvedScope(fs, session, fallbackWorkspaceRoot);
    };

    fs.stat = async (target, signal) => {
      const scope = await resolveCurrent();
      if (scope !== undefined && this.classify(fs, scope, target) === "denied")
        this.deny();
    ...
    fs.lstat = async (path, options, signal) => {
      ...
      if (scope !== undefined) {
        const target = await fs.resolve(path, {
          cwd: options?.cwd ?? scope.scope.workspaceRoot,
          signal,
        });
    ...
    fs.readText = async (target, signal) => {
      const scope = await resolveCurrent();
      if (scope !== undefined && this.classify(fs, scope, target) !== "content")
        this.deny();
    ...
    fs.listDir = async (target, signal) => {
      const scope = await resolveCurrent();
      if (scope === undefined || scope.scope.mode === "full")
        return original.listDir(target, signal);
