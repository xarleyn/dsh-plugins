...
function fakeFileSystem(): ScopeAwareFileSystem {
    ...
    [
      ...
      [
        { name: "apps", type: "directory", target: target(apps) },
        {
          name: "infrastructure",
          type: "directory",
          target: target(`${workspace}${sep}infrastructure`),
        },
      ],
    ],
    [
  ...
  return {
    ...
    contains: (parent, child) =>
      isLexicallyUnder(String(child.targetKey), String(parent.targetKey)),
    stat: vi.fn(async () => ({ type: "file" })),
    lstat: vi.fn(async () => ({ type: "file" })),
    readText: vi.fn(async (value) => `read:${value.displayPath}`),
    streamText: vi.fn(async (value) =>
      (async function* () {
        yield `read:${value.displayPath}`;
      })(),
    ),
...
describe("filesystem enforcement", () => {
  ...
  test("allows content reads and denies hidden siblings", async () => {
    ...
    await expect(
      runtime.run(session, () =>
        fs.readText(target(`${projectA}${sep}visible.txt`)),
      ),
    ).resolves.toContain("visible.txt");
    await expect(
      runtime.run(session, () =>
        fs.readText(target(`${projectB}${sep}hidden.txt`)),
      ),
  ...
  test("applies the same boundary to raw byte reads", async () => {
    ...
    await expect(
      runtime.run(session, () =>
        fs.readBytes(target(`${projectA}${sep}visible.png`), undefined, 1024),
      ),
    ).resolves.toEqual(new Uint8Array([1, 2, 3]));
    await expect(
      runtime.run(session, () =>
        fs.readBytes(target(`${projectB}${sep}hidden.png`), undefined, 1024),
      ),
    ).rejects.toMatchObject({
      code: SESSION_SCOPE_ERROR.DENIED,
    });
  ...
  test("allows navigation metadata but not navigation content", async () => {
    ...
    await expect(
      runtime.run(session, () => fs.stat(target(apps))),
    ).resolves.toBeDefined();
    await expect(
      runtime.run(session, () => fs.readText(target(apps))),
    ).rejects.toMatchObject({
      code: SESSION_SCOPE_ERROR.DENIED,
    });
  });
  ...
  test("leaves paths outside the session workspace to ordinary DSH policy", async () => {
    ...
    await expect(
      runtime.run(session, () => fs.readText(target(external))),
  ...
  test("denies a workspace path whose filesystem identity escapes through a symlink", async () => {
    ...
    await expect(
      runtime.run(session, () => fs.readText(escaped)),
    ).rejects.toMatchObject({
      code: SESSION_SCOPE_ERROR.DENIED,
    });
  });
  ...
  test("filters each navigation listing before returning names", async () => {
    ...
    const rootEntries = await runtime.run(session, () =>
      fs.listDir(target(workspace)),
    );
    const appEntries = await runtime.run(session, () =>
      fs.listDir(target(apps)),
    );
  ...
  test("keeps concurrent session scopes isolated", async () => {
    ...
    const [a, b] = await Promise.all([
      runtime.run(sessionA, () =>
        fs.readText(target(`${projectA}${sep}a.txt`)),
      ),
      runtime.run(sessionB, () =>
        fs.readText(target(`${projectB}${sep}b.txt`)),
      ),
    ]);
