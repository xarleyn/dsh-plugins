...
export type WorkerValidationFailure =
  "empty-output" | "output-too-large" | "insufficient-reduction";

export type WorkerValidation =
  | { readonly ok: true; readonly text: string }
  | { readonly ok: false; readonly reason: WorkerValidationFailure };

export function validateWorkerOutput(
  text: string,
  inputBytes: number,
  validation: ResolvedToolOffloadConfig["validation"],
): WorkerValidation {
  if (text.trim() === "") return { ok: false, reason: "empty-output" };
  const outputBytes = byteLength(text);
  if (outputBytes > validation.maxOutputBytes)
  ...
  if (validation.requireReduction) {
    ...
    if (outputBytes > maxAllowed)
      return { ok: false, reason: "insufficient-reduction" };
  }
  return { ok: true, text };
}
