...
describe("validateWorkerOutput", () => {
  it("accepts a compact, smaller answer", () => {
    const outcome = validateWorkerOutput(
  ...
  it("rejects empty and whitespace-only answers", () => {
    expect(validateWorkerOutput("", 50_000, validation)).toEqual({
      ok: false,
      reason: "empty-output",
    });
    expect(validateWorkerOutput("   \n\t", 50_000, validation)).toEqual({
      ok: false,
      reason: "empty-output",
    });
  });
  ...
  it("rejects answers that did not shrink the input by the minimum ratio (SPEC §9.6)", () => {
    ...
    expect(
      validateWorkerOutput(almostAsBig, Buffer.byteLength(input), validation),
    ).toEqual({
      ok: false,
  ...
  it("skips the reduction check when requireReduction is false", () => {
    const relaxed = testConfig({
      validation: { requireReduction: false },
    }).validation;
    const text = makeText(9_900);
    expect(
      validateWorkerOutput(text, Buffer.byteLength(text), relaxed),
    ).toEqual({ ok: true, text });
  });

  it("rejects an answer for an empty input when reduction is required", () => {
    expect(validateWorkerOutput("any", 0, validation)).toEqual({
      ok: false,
      reason: "insufficient-reduction",
    });
  });
