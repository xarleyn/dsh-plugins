...
function reportText(report: ScanReport): string {
  return [
    ...
    report.sessionsFailed === 0
      ? undefined
      : `${report.sessionsFailed} session(s) failed; see plugin logs.`,
  ]
  ...
  return {
    ...
    async handler(invocation: Invocation): Promise<CommandResult> {
      ...
      if (cwd === undefined || cwd.length === 0) {
        return {
          kind: "error",
          text: "This session has no workspace directory.",
        };
      }
      ...
      if (subcommand === "scan") {
        ...
        if (
          ...
          return {
            kind: "error",
            text: "Session limit must be a positive integer.",
          };
        }
        try {
          return {
      ...
      if (subcommand === "review") {
        ...
        if (records.length === 0) {
          return {
            kind: "success",
            text: "No correction evidence is stored for this workspace.",
          };
        }
        return {
          kind: "success",
