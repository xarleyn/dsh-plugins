...
Если Confluence смонтирован под context path (`https://<host>/wiki/...` — так
делают, когда вики и Jira живут на одном хосте), `allowPaths` должны
начинаться с этого префикса: шаблон `/pages/**` не совпадает с
`/wiki/pages/viewpage.action`, и страница уйдёт в сырой HTML. Тогда список
такой:
...
Важно: ссылка из адресной строки Confluence Server (`viewpage.action?pageId=`)
распознаётся именно как страница. Если такая ссылка попадает в правило без
Confluence-адаптера (или в правило Jira на том же хосте), в ответ уйдёт сырой
HTML страницы — шапка вики, меню, хлебные крошки и футер Atlassian, — а не
разобранный текст.

### 5.1 Уровень обрезки служебного (`cleanup`)

Тело страницы Confluence содержит не только текст: макросы оглавления,
списки дочерних страниц, вложения, статусы, эмотиконы, layout-обёртки.
Параметр `adapter.cleanup` решает, сколько этого служебного слоя попадёт в
ответ модели. Полезный контент — заголовки, абзацы, списки, таблицы, код,
панели, ссылки — рендерится одинаково на всех уровнях.

| Уровень | Навигационные макросы¹ | Неизвестные макросы | Вложения и картинки | Эмотиконы |
| --- | --- | --- | --- | --- |
| `off` — не режем | `_[macro: toc — 2]_` | имя и параметры | ссылка на файл (маркер — только если URL неизвестен) | имя |
| `balanced` — по умолчанию | удаляются | `_[macro: name]_` | ссылка на файл (маркер — только если URL неизвестен) | имя |
| `strict` — только контент | удаляются | тело без маркера | удаляются | удаляются |

¹ Оглавление, список дочерних страниц, список вложений, поиск и прочие
макросы-агрегаторы: они не несут собственного содержимого. Включения чужих
страниц (`include`, `excerpt-include`) остаются видимы как
`_[includes: Page]_` на уровнях `off`/`balanced` — это единственный след
текста, которого нет в теле страницы.

В карточке настроек уровень выбирается в поле **Page cleanup** рядом с типом
адаптера. Декларативно:
...
### 5.2 Вложения страницы и чтение документов

Тело страницы называет вложение только именем файла
(`<ri:attachment ri:filename="Регламент.docx"/>`), и раньше это имя доезжало до
модели без ссылки — забрать файл было нечем. Теперь адаптер:

- превращает каждую ссылку на вложение (обычная ссылка, картинка, макросы
  просмотра `view-file`/`viewpdf`/`viewdoc`/`viewppt`/`viewxls`) в ссылку на
  скачивание `<context>/download/attachments/<pageId>/<имя>?api=v2`;
- дописывает в конец страницы раздел `## Attachments` — тот же список, что
  показывает вкладка «Вложения» в Confluence: имя, тип, размер и ссылка. Так
  документ, который упомянут в тексте, но не связан ссылкой, тоже находится.
...
число строк; остаток указывается числом, а не пропадает молча. Если запрос
списка не удался, страница всё равно отдаётся — с пометкой
`_[attachment list unavailable]_`. Уровень `strict` не отдаёт ни ссылок, ни
списка.

Скачать вложение как бинарь нельзя: тело ответа в харнессе — закрытая пара
`html | text`, поэтому плагин извлекает ТЕКСТ документа у себя и отдаёт модели
уже Markdown. Работает это для Word (`.docx`/`.docm`/`.dotx`) и
OpenDocument (`.odt`): архив распаковывается в памяти (внешних программ и
временных файлов нет), из тела берутся заголовки, абзацы, пункты списков и
таблицы. Колонтитулы, сноски, комментарии, картинки и коды полей не
переносятся.

Ограничения задаются на правило (и наследуются из верхнего уровня `documents`):

```yaml
        documents:
          enabled: true       # по умолчанию true
          maxBytes: 4194304   # сколько байт вложения качать под извлечение (4 МиБ)
          maxChars: 40000     # сколько текста документа отдать модели
```

- вложение больше `maxBytes` → ошибка `AUTH_FETCH_DOCUMENT_TOO_LARGE`
  (в тексте ошибки сказано, какой лимит поднять);
- текст длиннее `maxChars` → обрезается, в конце стоит явная строка
  `_[text truncated at N characters …]_`;
- `.pdf`, `.doc`, `.xlsx`, `.pptx`, архивы → `AUTH_FETCH_DOCUMENT_UNREADABLE`
  с указанием формата, а не безликое «unsupported content type»;
- если под видом документа отдана HTML-страница (например, страница входа) —
  тоже `AUTH_FETCH_DOCUMENT_UNREADABLE`, потому что отдавать мусор текстом хуже,
  чем отказать.

`documents.enabled: false` возвращает прежнее поведение: любой не-текстовый
ответ — `AUTH_FETCH_UNSUPPORTED_CONTENT`. Кнопка **Test** на URL вложения
показывает ровно тот текст, который получит модель.
...
| `AUTH_FETCH_UNSUPPORTED_CONTENT` на URL вложения | Извлечение документов выключено (`documents.enabled: false`) или ответ — не Word/OpenDocument. Проверьте `documents` в правиле |
| `AUTH_FETCH_DOCUMENT_UNREADABLE` | Формат не читается как текст (`.pdf`, `.doc`, `.xlsx`, `.pptx`, архив) либо под видом документа отдана HTML-страница входа. Скажите оператору, какой формат нужен; для PDF нужен отдельный конвертер, которого у плагина нет |
| `AUTH_FETCH_DOCUMENT_TOO_LARGE` | Вложение больше `documents.maxBytes` (по умолчанию 4 МиБ); поднимите лимит в правиле осознанно — текст целиком уйдёт в контекст |
| В разделе `## Attachments` стоит `_[attachment list unavailable]_` | Запрос списка вложений не удался (нет прав на `/rest/api/content/**/child/attachment` или запрос ушёл не туда); текст страницы при этом получен |
