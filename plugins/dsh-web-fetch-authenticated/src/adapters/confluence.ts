...
function renderMacro(node: MarkupElement): string {
  ...
  const parameter = (parameterName: string): string | undefined => {
    for (const child of node.children) {
      if (
        ...
        child.attrs["ac:name"] === parameterName
      ) {
  ...
  if (name === "code") {
    const language =
      parameter("language") ?? parameter("syntaxhighlighter-parameter") ?? "";
    const bodyElement = node.children.find(
      (child) => isElement(child) && child.name === "ac:plain-text-body",
    );
    const body =
      bodyElement === undefined || !isElement(bodyElement)
        ? ""
        : textContent(bodyElement.children).replace(/\n$/u, "");
