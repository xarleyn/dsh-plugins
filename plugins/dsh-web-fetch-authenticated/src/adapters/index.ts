...
export async function applyAdapter(
  url: URL,
  context: AdapterRequestContext,
  fetchImpl: TransportFetch = (restUrl, restContext) =>
    authenticatedFetch({ url: restUrl, ...restContext }),
  ...
  const fetchJson = async (
    ...
    if (!isJsonBody(result.body.kind)) {
      throw errors.adapterFailed(
        `the REST endpoint ${restUrl.pathname} did not return JSON (content was ${result.body.kind})`,
      );
    }
    try {
      return {
        statusCode: result.statusCode,
        data: JSON.parse(result.body.content) as unknown,
      };
    } catch (cause) {
      throw errors.adapterFailed(
        `the REST endpoint ${restUrl.pathname} returned malformed JSON`,
        cause,
      );
    }
  };

  const { statusCode, markdown } =
    adapter.type === "jira"
...
function adapterApplies(adapter: ResolvedAdapter, url: URL): boolean {
  if (adapter.type === "jira") {
    ...
    return /(?:^|\/)(?:browse|issues)\/[A-Z][A-Z0-9]*-\d+$/iu.test(
      url.pathname,
    );
  }
