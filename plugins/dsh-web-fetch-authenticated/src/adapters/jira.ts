...
export function extractIssueKey(pathname: string): string | undefined {
  for (const segment of pathname.split("/")) {
    if (
      segment.length > 0 &&
      ISSUE_KEY_PATTERN.test(segment.toUpperCase()) &&
      segment.includes("-")
    ) {
...
export function commentApiPath(key: string, adapter: ResolvedAdapter): string {
  return adapter.jiraFlavor === "cloud"
...
function fieldName(
  field: { readonly name?: unknown } | null | undefined,
): string | undefined {
...
export async function fetchIssueMarkdown(
  ...
  if (key === undefined)
    throw errors.adapterFailed(
      `URL path "${requestUrl.pathname}" does not contain a Jira issue key`,
    );
  ...
  return {
    statusCode: response.statusCode,
    markdown: renderIssue(key, issue, adapter),
  };
}
...
function asIssue(
  data: unknown,
  path: string,
): { key?: string; fields: JiraIssueFields } {
  if (typeof data !== "object" || data === null) {
    throw errors.adapterFailed(
      `Jira REST response at ${path} is not a JSON object`,
    );
  }
  const record = data as { key?: unknown; fields?: unknown };
  if (typeof record.fields !== "object" || record.fields === null) {
    throw errors.adapterFailed(
      `Jira REST response at ${path} has no fields object`,
    );
  }
  return {
...
function renderIssue(
  ...
  lines.push(
    `# ${issue.key ?? key}${summary.length > 0 ? `: ${summary}` : ""}`,
  );
  lines.push("");
  const facts: string[] = [];
  ...
  if (Array.isArray(fields.labels) && fields.labels.length > 0) {
    facts.push(
      `Labels: ${fields.labels.filter((label): label is string => typeof label === "string").join(", ")}`,
    );
  }
    ...
    const names = fields.components
      .map((component) =>
        typeof component === "object" && component !== null
          ? (component as { name?: unknown }).name
          : undefined,
      )
      .filter((name): name is string => typeof name === "string");
    if (names.length > 0) facts.push(`Components: ${names.join(", ")}`);
  }
  if (typeof fields.created === "string")
    facts.push(`Created: ${fields.created.slice(0, 10)}`);
  if (typeof fields.updated === "string")
    facts.push(`Updated: ${fields.updated.slice(0, 10)}`);
  if (facts.length > 0) {
...
function renderComments(comment: unknown): string[] {
  ...
  for (const entry of comments) {
    ...
    const date =
      typeof record.created === "string" ? record.created.slice(0, 10) : "";
...
interface AdfNode {
  ...
  readonly marks?:
    | ReadonlyArray<{
        readonly type?: unknown;
        readonly attrs?: { readonly href?: unknown } | undefined;
      }>
    | undefined;
...
function adfChildren(node: AdfNode): AdfNode[] {
  return Array.isArray(node.content)
    ? node.content
        .map(adfNode)
        .filter((child): child is AdfNode => child !== undefined)
    : [];
}
...
function adfBlock(node: AdfNode): string {
  switch (node.type) {
    ...
    case "mediaSingle": {
      ...
      const alt =
        typeof media?.attrs?.text === "string"
          ? media.attrs.text
          : "attachment";
      return `_[${alt}]_`;
    }
  ...
  items.forEach((item, index) => {
    if (item.type !== "listItem") {
      ...
      if (text.trim().length > 0)
        out.push(`${"  ".repeat(depth)}${marker} ${text.trim()}`);
      return;
    }
    const parts = adfChildren(item);
    const first = parts[0];
    const text =
      first === undefined ? "" : adfBlock(first).replace(/\n/gu, " ").trim();
...
function adfTable(table: AdfNode): string {
  ...
  const cellsOf = (row: AdfNode): string[] =>
    adfChildren(row).map((cell) =>
      adfInline(adfChildren(cell)).replace(/\|/gu, "\\|").trim(),
    );
