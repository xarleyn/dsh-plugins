...
export function resolveHostname(hostname: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    lookup(hostname, { all: true, verbatim: true }, (error, addresses) => {
      ...
      resolve(
        (addresses as readonly LookupAddress[]).map((entry) => entry.address),
      );
    });
  });
}
...
export async function resolveApprovedAddresses(
  ...
  try {
    ...
    throw new AddressResolutionError(
      `DNS resolution failed for "${hostname}": ${describeError(error)}`,
    );
  }
  if (answers.length === 0) {
    throw new AddressResolutionError(
