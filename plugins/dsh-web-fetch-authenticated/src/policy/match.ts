...
export function ruleMatchesUrl(
  ...
  if (
    match.denyPaths !== undefined &&
    match.denyPaths.some((pattern) => pathMatches(pattern, pathname))
  ) {
    return false;
  }
...
export function matchRules(
  ...
  return rules.filter(
    (rule) => rule.source.enabled && ruleMatchesUrl(rule.source, url),
  );
}
