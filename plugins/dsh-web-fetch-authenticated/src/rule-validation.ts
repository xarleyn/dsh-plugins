...
export function validateRule(
  ...
  if (!Array.isArray(hosts) || hosts.length === 0) {
    ...
    for (const host of hosts) {
      if (typeof host !== "string" || !isValidConfiguredHost(host)) {
        errors.push(
          `${label}: host "${String(host)}" is not a valid exact hostname (wildcards are not supported in v1)`,
        );
      }
    }
  }
    ...
    if (!Array.isArray(schemes)) {
      ...
      for (const scheme of schemes) {
        if (scheme !== "https" && scheme !== "http")
          errors.push(`${label}: unsupported scheme "${String(scheme)}"`);
      }
    }
  }
    ...
    if (!Array.isArray(ports)) {
      ...
      for (const port of ports) {
        if (
          ...
          errors.push(
            `${label}: port ${String(port)} is not an integer in 1..65535`,
          );
        }
      }
    }
  ...
  for (const [field, patterns] of [
    ["allowPaths", match.allowPaths],
    ["denyPaths", match.denyPaths],
  ] as const) {
    if (
      patterns === undefined ||
    ...
    for (const pattern of patterns) {
      if (
        ...
        compilePathPattern(pattern) === undefined
      ) {
        errors.push(
          `${label}: malformed ${field} pattern "${String(pattern)}" (must start with "/")`,
        );
      }
    }
  }
  ...
  if (limits !== undefined) {
    if (limits.timeoutMs !== undefined && !isPositiveFinite(limits.timeoutMs))
      errors.push(`${label}: limits.timeoutMs must be a positive number`);
    if (
      limits.maxResponseBytes !== undefined &&
      !isPositiveFinite(limits.maxResponseBytes)
    )
      errors.push(
        `${label}: limits.maxResponseBytes must be a positive number`,
      );
    if (
      limits.maxBodyChars !== undefined &&
      !isPositiveFinite(limits.maxBodyChars)
    )
      errors.push(`${label}: limits.maxBodyChars must be a positive number`);
  }
  if (rule.testUrl !== undefined && rule.testUrl.length > 0) {
    try {
      ...
      if (testUrl.protocol !== "https:" && testUrl.protocol !== "http:")
        errors.push(`${label}: testUrl must be http(s)`);
    } catch {
...
function validateAuth(auth: AuthConfig | undefined, label: string): string[] {
  ...
  if (type === "bearer") {
    if (!isCredentialRef(auth.credential))
      errors.push(
        `${label}: bearer auth requires a valid credential reference name`,
      );
  } else if (type === "basic") {
    if (typeof auth.username !== "string" || auth.username.length === 0)
      errors.push(`${label}: basic auth requires a username`);
    if (!isCredentialRef(auth.passwordCredential))
      errors.push(
        `${label}: basic auth requires a valid password credential reference name`,
      );
  } else {
    if (
      typeof auth.headerName !== "string" ||
      !HEADER_NAME_PATTERN.test(auth.headerName)
    ) {
      ...
      errors.push(
        `${label}: header name "${auth.headerName}" is a forbidden transport header`,
      );
    }
    if (!isCredentialRef(auth.credential))
      errors.push(
        `${label}: header auth requires a valid credential reference name`,
      );
    if (auth.prefix !== undefined && typeof auth.prefix !== "string")
      errors.push(`${label}: header prefix must be a string when present`);
  }
  return errors;
}
...
function validateNetworkPolicy(
  ...
  for (const [field, cidrs] of [
    ...
    if (cidrs === undefined || (Array.isArray(cidrs) && cidrs.length === 0))
      continue;
    if (!Array.isArray(cidrs)) {
      errors.push(
        `${label}: networkPolicy.${field} must be an array when present`,
      );
      continue;
    }
    for (const cidr of cidrs) {
      if (typeof cidr !== "string" || !isCidr(cidr))
        errors.push(
          `${label}: malformed networkPolicy.${field} CIDR "${String(cidr)}"`,
        );
    }
  }
  return errors;
...
function validateRedirectPolicy(
  policy: RedirectPolicy | undefined,
  label: string,
): string[] {
  if (policy === undefined) return [];
  const errors: string[] = [];
  ...
  if (
    ...
    errors.push(
      `${label}: redirect mode must be none, same-origin, or allowlist`,
    );
  }
  if (policy.maxRedirects !== undefined) {
    ...
    if (
      ...
      errors.push(
        `${label}: redirects.maxRedirects must be a non-negative integer`,
      );
    }
  }
  ...
  if (mode === "allowlist") {
    ...
    if (!Array.isArray(origins) || origins.length === 0) {
      errors.push(
        `${label}: redirect mode allowlist requires a non-empty allowedOrigins list`,
      );
    } else {
      for (const origin of origins) {
        if (!isValidOrigin(origin))
          errors.push(
            `${label}: redirect allowedOrigins entry "${String(origin)}" is not an http(s) origin`,
          );
      }
    }
  } else if (
  ...
  for (const [index, rule] of rules.entries()) {
    ...
    if (rule.enabled !== false) {
      ...
      if (schemes !== undefined && schemes.includes("http")) {
        warnings.push(
          `rule "${rule.id}": http:// is allowed; responses travel unencrypted`,
        );
      }
      const policy = rule.networkPolicy;
      if (
        policy?.allowPrivate === true &&
        (policy.allowedCidrs === undefined || policy.allowedCidrs.length === 0)
      ) {
        warnings.push(
          `rule "${rule.id}": all RFC1918 private ranges are allowed; prefer narrow allowedCidrs`,
        );
      }
      ...
      if (
        rule.match.allowPaths === undefined ||
        rule.match.allowPaths.length === 0
      ) {
        warnings.push(
          `rule "${rule.id}": every path on the matched origin is allowed; add allowPaths to narrow it`,
        );
      }
    }
  }
