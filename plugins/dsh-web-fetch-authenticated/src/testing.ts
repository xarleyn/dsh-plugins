...
async function credentialState(
  ...
  rule: ResolvedRule,
): Promise<CredentialStateView | undefined> {
...
function credentialStateFields(view: CredentialStateView | undefined): {
  credentialState?: CredentialStateView;
} {
...
async function classifyDns(
  ...
  try {
    const { verdicts } = await resolveApprovedAddresses(
      url.hostname,
      rule.networkPolicy,
    );
    return {
...
export async function testRule(
  ...
  if (rule === undefined || rule.source.enabled === false) {
    return {
      ...base,
      outcome: "AUTH_FETCH_RULE_DISABLED",
      detail: "the rule is disabled or missing",
      durationMs: elapsed(),
    };
  }
  const urlText =
    requestedUrl !== undefined && requestedUrl.length > 0
      ? requestedUrl
      : (rule.source.testUrl ?? "");
  let url: URL;
  try {
  ...
  const matchedThis = matched.some(
    (candidate) => candidate.source.id === ruleId,
  );
  ...
  if (!matchedThis) {
    return {
      ...
      outcome: "AUTH_FETCH_NO_MATCHING_RULE",
      detail:
        "the URL does not match this rule (check hosts, ports, schemes, and path patterns)",
      durationMs: elapsed(),
    };
  }
  if (!dns.allowed) {
  ...
  try {
    ...
    const outcome =
      detail.includes("credential") && credentialStateView?.configured === false
    ...
    matched.length === 0
      ...
      : matched.length > 1
        ...
        : matched[0] === undefined
          ...
          : {
              ruleId: matched[0].source.id,
              ruleName: matched[0].source.name,
              reason: "matched",
            };
