...
import { configWith, fixtureRule, startFixture, type FixtureServer } from './helpers.js'
 ...
 */
function providerFake(values: Record<string, string>): NonNullable<CredentialsServiceLike> {
  return {
...
describe('credential resolver', () => {
  test('reads the provider per operation, so one that mounts later is not lost', async () => {
    ...
    const holder: { provider: CredentialsServiceLike } = { provider: undefined }
    ...
    expect(await resolver.describe('TEST_TOKEN')).toEqual({ configured: true, writable: true })
  })
  ...
  test('an empty stored value reads as absent and a malformed name never resolves', async () => {
    const resolver = createCredentialResolver(() => providerFake({ EMPTY_TOKEN: '', 'BAD-NAME': SECRET }))
    expect(await resolver.resolve('EMPTY_TOKEN')).toBeUndefined()
    expect(await resolver.describe('EMPTY_TOKEN')).toEqual({ configured: false, writable: true })
...
describe('plugin credential wiring', () => {
  test('credentials mounted after construction still authorize a test fetch', async () => {
    const server = await startFixture({ '/secure/data': { body: '{"ok":true}' } })
    servers.push(server)
