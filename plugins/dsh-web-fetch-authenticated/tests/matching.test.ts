...
describe("rule matching order", () => {
  test("scheme, host, port, deny, allow", () => {
    ...
    expect(
      ruleMatchesUrl(rule, url("https://jira.example.corp:8443/browse/PROJ-1")),
    ).toBe(true);
    expect(
      ruleMatchesUrl(rule, url("http://jira.example.corp:8443/browse/PROJ-1")),
    ).toBe(false);
    expect(
      ruleMatchesUrl(rule, url("https://jira.example.corp/browse/PROJ-1")),
    ).toBe(false);
    expect(
      ruleMatchesUrl(
        rule,
        url("https://jira.example.corp:8443/browse/admin/x"),
      ),
    ).toBe(false);
    expect(
      ruleMatchesUrl(rule, url("https://jira.example.corp:8443/other")),
    ).toBe(false);
  });
  ...
  test("hostname normalization is exact, not substring", () => {
    ...
    expect(ruleMatchesUrl(rule, url("https://xjira.example.corp/x"))).toBe(
      false,
    );
  });
  ...
  test("no allowPaths allows every path on the origin", () => {
    const rule = fixtureRule("http://127.0.0.1:1", {
      match: { hosts: ["svc.example"] },
    });
    expect(ruleMatchesUrl(rule, url("https://svc.example/any/thing"))).toBe(
      true,
    );
  });
});
...
describe("config resolution", () => {
  test("invalid rules are dropped and reported", () => {
    ...
    const bad = fixtureRule("http://127.0.0.1:1", {
      id: "bad",
      match: { hosts: [] },
    });
    ...
    expect(resolved.configErrors.some((error) => error.includes("bad"))).toBe(
      true,
    );
  });
  ...
  test("rule limits override global limits", () => {
    ...
    const resolved = resolveConfig(
      configWith([rule], { limits: { timeoutMs: 9999, maxBodyChars: 555 } }),
    );
  ...
  test("validateRule flags a missing credential reference grammar", () => {
    const rule = fixtureRule("http://127.0.0.1:1", {
      auth: { type: "bearer", credential: "not a ref" },
    });
    expect(
      validateRule(rule, 0).some((error) =>
        error.includes("credential reference"),
      ),
    ).toBe(true);
  });
  ...
  test("schemastery-normalized empty arrays read as absent (round-trip through settings)", () => {
    ...
    expect(
      matchRules(resolved.rules, url("http://jira.example.corp/x")),
    ).toHaveLength(0);
    expect(
      matchRules(resolved.rules, url("https://jira.example.corp/x")),
    ).toHaveLength(1);
  });

  test("matchRules ignores disabled rules even when the URL fits", () => {
    const resolved = resolveConfig(
      configWith([fixtureRule("http://127.0.0.1:1", { enabled: false })]),
    );
    expect(
      matchRules(resolved.rules, url("http://127.0.0.1:1/secure/x")),
    ).toHaveLength(0);
  });
});
